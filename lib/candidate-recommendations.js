...
async function idsForExpandTarget(gap) {
  const bpmIds = await unionSetKeys(
    bpmBucketsForRange(gap.target_bpm_range || gap.bpm_range).map(
      (bucket) => BPM_INDEX_PREFIX + bucket
    )
  ...
  if (bpmIds.length && camelotIds.length && familyIds.length) {
    const exact = intersectSets([bpmIds, camelotIds, familyIds]);
    if (exact.length) return { ids: exact, match_scope: 'context_bpm_camelot_family' };
  }
  if (bpmIds.length && familyIds.length) {
    const bpmFamily = intersectSets([bpmIds, familyIds]);
    if (bpmFamily.length) return { ids: bpmFamily, match_scope: 'context_bpm_family' };
  }
  if (camelotIds.length && familyIds.length) {
    const camelotFamily = intersectSets([camelotIds, familyIds]);
    if (camelotFamily.length) return { ids: camelotFamily, match_scope: 'context_camelot_family' };
  }
  if (familyIds.length) return { ids: familyIds, match_scope: 'context_family' };
  if (bpmIds.length) return { ids: bpmIds, match_scope: 'context_bpm' };
  if (camelotIds.length) return { ids: camelotIds, match_scope: 'context_camelot' };
  return { ids: [], match_scope: 'empty' };
}

async function idsForGap(gap) {
  if (gap && gap.type === 'expand') return await idsForExpandTarget(gap);
...
function analyzeCandidateRelease(release, index, gap, profileInput) {
  ...
  if (
    ...
    (tasteMatch || partialHit)
  ) {
    bucket = 'strong';
  } else if (metadataCoverage >= 0.75 && compatibilityScore >= 70 && closesGap) bucket = 'strong';
...
function whyForCandidate(release, analysis, gap) {
  ...
  } else if (gap && gap.type === 'expand') {
    why.push('ложится в сильную зону твоего ящика');
...
function groupSummary(gap, strong, probable, explore) {
  if (strong.length) {
    ...
    if (gap && gap.type === 'expand') {
      return (
        'Нашёл ' + strong.length + ' сильн. кандидата, которые ложатся в твой текущий контекст.'
      );
    }
