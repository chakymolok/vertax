...
/* Heuristic genre family classifier. Order matters: more specific scenes
 * before the generic 'bass' bucket. Use this list as the source of truth
 * — Discogs styles can be weirdly formatted ("D&B", "Drum'n'Bass",
 * "Drum + Bass" all seen in the wild) so we match with regexes that
 * tolerate punctuation/whitespace between letters. */
function genreFamily(genre, subGenre, styles) {
  ...
  /* DnB family — broad match: Drum N Bass / D&B / Drum'n'Bass / DnB
   * plus the major sub-scenes Discogs tags separately. */
  const dnbRegex = /\bd[\s'’&+\-_/.]*n[\s'’&+\-_/.]*b(ass|\b)|\bdrum[\s'’&+\-_/.]*(?:n|and|&|\+)[\s'’&+\-_/.]*bass\b|\bd&b\b|\bdnb\b/;
  if (
    dnbRegex.test(haystack) ||
    haystack.includes('liquid funk') ||
    haystack.includes('liquid dnb') ||
    haystack.includes('neurofunk') ||
    haystack.includes('neuro funk') ||
    haystack.includes('jump up') ||
    haystack.includes('jumpup') ||
    haystack.includes('rollers') ||
    haystack.includes('autonomic') ||
    haystack.includes('minimal dnb')
  )
    return 'dnb_jungle';
  ...
  /* Halftime: lives between dnb and downtempo. Keep it as its own scene
   * so 170-BPM half-time DnB doesn't get mis-bucketed as ambient. */
  if (
    haystack.includes('halftime') ||
    haystack.includes('half time') ||
    haystack.includes('half-time')
  )
    return 'halftime';
  ...
  /* Word-boundary — naked 'electro' substring leaked from 'electronic'
   * (the generic Discogs genre) and swallowed everything. */
  if (
    /\belectro\b/.test(haystack) ||
    haystack.includes('breakbeat') ||
    /\bbreaks\b/.test(haystack)
  )
  ...
  if (haystack.includes('downtempo') || haystack.includes('ambient'))
  ...
  /* Bass scene — explicit only. Earlier this matched a naked 'drum'
   * substring, which leaked Drum N Bass releases here when the DnB
   * regex above couldn't fire. Replaced with explicit terms. */
  if (
    haystack.includes('uk bass') ||
    haystack.includes('bass / club') ||
    haystack.includes('wobble') ||
    haystack.includes('club bass')
  )
    return 'bass';
