...
async function syncPublicCatalogBatch(input) {
  ...
  const allReleaseIds = Array.from(loaded.groups.keys())
    .sort((a, b) => Number(a) - Number(b));
  const catalogIds = new Set(await listReleaseIds());
  const eligibleIds = missingOnly
    ? allReleaseIds.filter((id) => !catalogIds.has(id))
    : allReleaseIds;
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    try {
      ...
      const release = mergeCachedTracksIntoDiscogs(discogs, loaded.groups.get(releaseId));
