...
// Public HTTP reads must distinguish an outage from an empty catalog/404.
// Background ingestion retains its existing best-effort Redis behavior.
async function readCatalogArray(command, args, options) {
  if (!options || !options.strict) return await safeRedis(command, args, []) || [];
  const values = await redisCommand(command, args);
  if (!Array.isArray(values) || (command === 'MGET' && values.length !== args.length)) {
    throw new Error('catalog_storage_unavailable');
  }
  return values;
}

async function getPublicRelease(discogsId, options) {
  const id = String(discogsId || '').trim();
  if (!/^\d+$/.test(id)) return null;
  const values = await readCatalogArray(
    'MGET',
...
async function listReleaseIds(options) {
  const sets = await Promise.all([
    readCatalogArray('SMEMBERS', [PUBLIC_RELEASE_SET_KEY], options),
    readCatalogArray('SMEMBERS', [CANDIDATES_ALL_KEY], options),
  ]);
  return Array.from(
...
async function loadReleaseBatch(ids, options) {
  const out = [];
  for (let offset = 0; offset < ids.length; offset += 100) {
    ...
    const values = await readCatalogArray('MGET', keys, options);
...
async function listPublicReleases(input) {
  ...
  const ids = await listReleaseIds(options);
  const releases = (await loadReleaseBatch(ids, options))
...
async function listAllPublicReleases(options) {
  const ids = await listReleaseIds(options);
  return (await loadReleaseBatch(ids, options))
