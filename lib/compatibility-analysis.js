...
async function loadMarketplace(releaseId, release) {
  ...
  if (Number.isFinite(lowest) || Number.isFinite(num)) {
    return {
      ...
      price_source: Number.isFinite(lowest) ? 'discogs_lowest_price' : null
    };
  }
  try {
    ...
    return { lowest_price: null, currency: null, num_for_sale: null, price_source: null };
  }
}
...
function autoSummary(result) {
  ...
  if (market.lowest_price || market.num_for_sale) {
    bits.push('Маркетплейс: ' + (market.lowest_price ? 'от ' + market.lowest_price + (market.currency ? ' ' + market.currency : '') : 'цена неизвестна') + (market.num_for_sale ? ', ' + market.num_for_sale + ' в продаже' : '') + '.');
  }
  return bits.join(' ');
}
