...
function mapReleaseTrack(release, track, idx) {
  ...
  return {
    ...
    camelot: null,
    key: null,
    genre: firstGenre(release.genres || release.styles),
    enriched: false,
    source: null
  };
}
...
async function analyzeReleaseAgainstIndex(options) {
  ...
  const purchaseScore = Math.round(100 * (0.8 * (compatibilityScore / 100) + 0.2 * (ratingNorm == null ? 0.5 : ratingNorm)));
  ...
  const result = {
    ...
    scores: {
      ...
      recommended: compatibilityScore >= 55 && matchData.harmonicOverlap >= 0.25 && conf !== 'low',
      confidence: conf
    },
    breakdown: {
