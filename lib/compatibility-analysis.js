...
function normalizeCollectionTrack(track, idx) {
  ...
  return {
    id: cleanString(track.id, 160) || crypto.createHash('sha1').update(stableTrackKey(track) + '|' + idx).digest('hex'),
    artist,
    title,
...
function normalizeCollectionPayload(collection) {
  ...
  (Array.isArray(collection) ? collection : []).forEach((item, idx) => {
    if (Array.isArray(item && item.tracklist)) {
      ...
      const recordKey = releaseId || item.id || normalizeText((item.artist || '') + '|' + (item.title || ''));
...
function genreFamily(genre) {
  ...
  if (!text) return null;
  const families = {
    bass: ['drum bass', 'drum and bass', 'dnb', 'jungle', 'breaks', 'breakbeat', 'uk bass', 'footwork', 'uk garage', 'bassline', 'bass', 'club'],
    dub: ['dubstep', '140', 'deep dubstep', 'grime'],
    house_techno: ['house', 'deep house', 'techno', 'melodic house techno', 'nu disco', 'disco']
  };
  for (const family of Object.keys(families)) {
    if (families[family].some((item) => text === normalizeText(item) || text.indexOf(normalizeText(item)) >= 0)) {
      return family;
    }
  }
  return text;
...
async function discogsJson(path, params) {
  ...
  let data = {};
  try { data = text ? JSON.parse(text) : {}; } catch (_) { data = { error: 'discogs_bad_json' }; }
  if (!response.ok) {
...
async function loadMarketplace(releaseId, release) {
  ...
  let suggested = null;
  try {
    const suggestions = await discogsJson('/marketplace/price_suggestions/' + encodeURIComponent(releaseId), {});
    const prices = Object.keys(suggestions || {})
      .map((key) => suggestions[key] && suggestions[key].value != null ? Number(suggestions[key].value) : NaN)
    ...
    if (prices.length) {
      ...
      suggested = {
        ...
        currency: suggestions[Object.keys(suggestions)[0]] && suggestions[Object.keys(suggestions)[0]].currency || 'EUR',
  ...
  if (Number.isFinite(lowest) || Number.isFinite(num)) {
    return {
      ...
      price_source: suggested && suggested.price_source || (Number.isFinite(lowest) ? 'discogs_lowest_price' : null)
    };
  }
  try {
    ...
    return {
      average_price: suggested && suggested.average_price || null,
      median_price: suggested && suggested.median_price || null,
      lowest_price: stats && stats.lowest_price && stats.lowest_price.value != null ? Number(stats.lowest_price.value) : null,
      currency: suggested && suggested.currency || stats && stats.lowest_price && stats.lowest_price.currency || null,
      num_for_sale: stats && stats.num_for_sale != null ? Number(stats.num_for_sale) : null,
      price_source: suggested && suggested.price_source || (stats && stats.lowest_price ? 'discogs_marketplace_stats' : null)
    };
  } catch (_) {
    return {
      average_price: suggested && suggested.average_price || null,
...
function releaseArtist(release) {
  const artists = (release && release.artists || []).map((a) => String(a.name || '').replace(/\s\(\d+\)$/, '')).filter(Boolean);
...
function releaseCoverUrl(release) {
  ...
  return primary && (primary.uri || primary.resource_url || primary.uri150) || release && release.thumb || '';
}
...
function mapReleaseTrack(release, track, idx) {
  ...
  return {
    id: crypto.createHash('sha1').update(String(release.id || '') + '|' + (track.position || idx) + '|' + title).digest('hex'),
...
async function enrichReleaseTrack(release, track, manual) {
  ...
  if (cached && cached.body && cached.body.matched !== false && cached.body.bpm && cached.body.camelot) {
  ...
  try {
    const body = await enrichTrackFromBeatport(track.artist, track.title, '', releaseLabel(release));
...
async function enrichReleaseTracks(release, manualTracks) {
  const manual = manualMap(manualTracks);
  const rawTracks = (release.tracklist || [])
    .filter((track) => track && track.title && (track.position || track.type_ === 'track' || !track.type_))
...
function camelotCompatibility(a, b) {
  ...
  if (x.n === y.n && x.mode === y.mode) return { score: 1, reason: 'тот же Camelot ' + cleanCamelot(a) };
  if (x.mode === y.mode && wrappedDiff(x.n, y.n) === 1) return { score: 0.8, reason: 'соседний Camelot ' + cleanCamelot(b) };
  if (x.n === y.n && x.mode !== y.mode) return { score: 0.7, reason: 'смена лада ' + cleanCamelot(a) + ' → ' + cleanCamelot(b) };
  const boosted = ((x.n + 7 - 1) % 12) + 1;
  if (x.mode === y.mode && boosted === y.n) return { score: 0.5, reason: 'energy boost +7 по Camelot' };
...
function bpmCompatibility(a, b) {
  ...
  if (pct <= 0.03) return { score: 1, reason: 'BPM отличается на ' + Math.round(pct * 1000) / 10 + '%' };
  if (pct <= 0.06) return { score: 0.7, reason: 'BPM отличается на ' + Math.round(pct * 1000) / 10 + '%' };
  const ratios = [
...
function trackCompatibility(releaseTrack, collectionTrack) {
  ...
  if (cam.score === 0 || bpm.score === 0) {
    return { score: 0, camelotScore: cam.score, bpmScore: bpm.score, styleAffinity: style.score, reasons: [] };
  }
...
function candidateCollectionTracks(index, track) {
  ...
  return ids.size ? Array.from(ids).map((idx) => index.tracks[idx]).filter(Boolean) : index.tracks;
}

function analyzeMatches(releaseTracks, index) {
  const enrichedTracks = releaseTracks.filter((track) => track.enriched && track.bpm && track.camelot);
  const matches = [];
  const unmatched = [];
  const densities = [];
  ...
  enrichedTracks.forEach((track) => {
    ...
    if (best.length) {
      matches.push({
        ...
        best_collection_matches: best.map((item) => publicCollectionMatch(item.candidate, item.compatibility))
      });
    } else {
  ...
  const density = densities.length ? densities.reduce((sum, item) => sum + item, 0) / densities.length : 0;
...
function collectionProfile(index) {
  ...
  const top = (obj, limit) => Object.keys(obj)
...
function autoSummary(result) {
  ...
  if (result.scores.confidence === 'low') {
    bits.push('Оценка предварительная: BPM/Key найден только для ' + enriched + ' из ' + total + ' треков.');
  } else {
  ...
  if (top) {
    ...
    bits.push('Сильнейшее совпадение: ' + rt.title + ' (' + rt.camelot + ', ' + rt.bpm + ') ↔ ' + top.title + ' (' + top.camelot + ', ' + top.bpm + ').');
  }
  if (result.release.rating) {
    bits.push('Discogs: ' + result.release.rating + '/5' + (result.release.rating_count ? ' (' + result.release.rating_count + ' голосов)' : '') + '.');
  }
  ...
  if (market.average_price || market.lowest_price || market.num_for_sale) {
    ...
    bits.push('Цена на Discogs: ' + (price ? (market.average_price ? 'примерно ' : 'от ') + price + (market.currency ? ' ' + market.currency : '') : 'цена неизвестна') + (market.num_for_sale ? ', ' + market.num_for_sale + ' в продаже' : '') + '.');
  }
  return bits.join(' ');
}
...
async function analyzeReleaseAgainstIndex(options) {
  ...
  const releaseTracks = await enrichReleaseTracks(release, options.manual_tracks || []);
  const matchData = analyzeMatches(releaseTracks, index);
  const metadataCoverage = releaseTracks.length ? matchData.enrichedCount / releaseTracks.length : 0;
  const compatibilityScore = Math.round(100 * (0.65 * matchData.harmonicOverlap + 0.35 * matchData.density));
  const rating = release && release.community && release.community.rating && release.community.rating.average ? Number(release.community.rating.average) : null;
  const ratingCount = release && release.community && release.community.rating && release.community.rating.count ? Number(release.community.rating.count) : null;
  const ratingNorm = rating ? rating / 5 : null;
  let purchaseScore = Math.round(100 * (0.8 * (compatibilityScore / 100) + 0.2 * (ratingNorm == null ? 0.5 : ratingNorm)));
  ...
  const recommended = compatibilityScore >= 70 && matchData.harmonicOverlap >= 0.25 && conf !== 'low';
  const result = {
    release: {
      title: release.title || '',
      artist: releaseArtist(release),
      ...
      format: (release.formats || []).map((f) => [f.name].concat(f.descriptions || []).filter(Boolean).join(' ')).filter(Boolean).join(' / '),
      label: releaseLabel(release),
      genres: compactList(release.genres || []),
      styles: compactList(release.styles || []),
      notes: cleanString(release.notes, 900) || '',
      videos: (release.videos || []).slice(0, 5).map((video) => ({
    ...
    scores: {
      ...
      recommended,
      confidence: conf,
      recommendation_label: recommendationLabel(compatibilityScore, conf, matchData.harmonicOverlap)
    },
    breakdown: {
      ...
      matched_track_count: matchData.matches.filter((item) => item.best_collection_matches && item.best_collection_matches[0] && item.best_collection_matches[0].compatibility > 0.5).length,
    ...
    tracks_not_enriched: releaseTracks.filter((track) => !track.enriched || !track.bpm || !track.camelot).map(publicTrack)
  };
