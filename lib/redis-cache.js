...
function beatportPayloadFromRecord(record) {
  ...
  return normalizeBeatportPayload({
    ...
    sample_url: flat.sample_url || flat.preview_url || flat.audio_url || (typeof flat.sample === 'string' ? flat.sample : null) || null,
    sample_duration_ms: flat.sample_duration_ms == null ? null : flat.sample_duration_ms,
...
function mergeDiscogsPayload(record, discogs) {
  ...
  const incomingAdminMeta = String(discogs.meta_status || '').trim().toLowerCase() === 'admin';
  const incomingAdminBpm = isAdminSource(discogs.bpm_source) || incomingAdminMeta;
  const incomingAdminKey = isAdminSource(discogs.key_source) || incomingAdminMeta;
  const currentAdminMeta = String(current.meta_status || '').trim().toLowerCase() === 'admin';
  const currentAdminBpm = isAdminSource(current.bpm_source) || currentAdminMeta;
  const currentAdminKey = isAdminSource(current.key_source) || currentAdminMeta;
  const preserveAdminBpm = currentAdminBpm && !incomingAdminBpm;
  const preserveAdminKey = currentAdminKey && !incomingAdminKey;
  const adminMeta = incomingAdminBpm || incomingAdminKey || currentAdminBpm || currentAdminKey;
  ...
  const out = Object.assign({}, current, {
    ...
    genre: preferIncoming(discogs.genre, current.genre),
    sub_genre: preferIncoming(discogs.sub_genre, current.sub_genre),
    beatport_url: preferIncoming(discogs.beatport_url, current.beatport_url),
    ...
    bpm: preserveAdminBpm ? current.bpm : preferIncoming(discogs.bpm, current.bpm),
    key_name: preserveAdminKey
      ? current.key_name
      : preferIncoming(discogs.key_name, current.key_name),
    camelot: preserveAdminKey
      ? current.camelot
      : preferIncoming(discogs.camelot, current.camelot),
    bpm_source: preserveAdminBpm
      ? current.bpm_source
      : preferIncoming(discogs.bpm_source, current.bpm_source),
    key_source: preserveAdminKey
      ? current.key_source
      : preferIncoming(discogs.key_source, current.key_source),
    confidence: preserveAdminBpm || preserveAdminKey
      ? current.confidence
      : preferIncoming(discogs.confidence, current.confidence),
    meta_status: adminMeta
      ? 'admin'
      : preferIncoming(discogs.meta_status, current.meta_status),
    original_bpm: preserveAdminBpm
      ? current.original_bpm
      : preferIncoming(discogs.original_bpm, current.original_bpm),
    halftime_corrected: preserveAdminBpm
      ? Boolean(current.halftime_corrected)
      : preferIncoming(discogs.halftime_corrected, current.halftime_corrected),
    sources: mergeArrays(sources, discogs.sources || discogs.source),
    source: adminMeta ? 'admin' : (discogs.source || current.source || 'discogs'),
