...
async function callGeminiModel(summary, model) {
  ...
  if (!response.ok) {
    ...
    err.code = response.status === 429 ? 'ai_rate_limited' : response.status === 403 ? 'ai_forbidden' : 'ai_verdict_failed';
    throw err;
  }
  ...
  return { text: text.slice(0, 1400), model };
}
...
async function callGemini(summary) {
  ...
  for (const model of models) {
    try {
      ...
      lastError = error;
      if (error && (error.status === 400 || error.status === 404 || error.status === 503)) continue;
      throw error;
    }
  }
  throw lastError || Object.assign(new Error('gemini_failed'), { status: 502, code: 'ai_verdict_failed' });
}
...
async function getAiVerdict(payload) {
  ...
  const gemini = await callGemini(summary);
  const body = {
    verdict: gemini.text,
    provider: 'gemini',
    model: gemini.model,
