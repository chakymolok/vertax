...
function compactAnalysisSummary(input) {
  ...
  return {
    ...
    collection_profile: {
      ...
      top_labels: Array.isArray(collectionProfile.top_labels) ? collectionProfile.top_labels.slice(0, 10) : [],
      top_artists: Array.isArray(collectionProfile.top_artists) ? collectionProfile.top_artists.slice(0, 10) : []
    },
...
function buildPrompt(summary, lang) {
  ...
  return [
    ...
    '- Сравни релиз с collection_profile: диапазон BPM, жанровые семьи, основные жанры, топ лейблов и топ артистов коллекции. Если релиз выглядит чужеродным, скажи это прямо.',
    '- Если лейбл или артист релиза уже близок к топ лейблам/артистам коллекции, упомяни это как плюс контекста. Если нет — не притягивай.',
