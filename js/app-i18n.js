...
(function () {
  ...
  var extraText = {
    en: {
      ...
      'Параметры сборки': 'Build settings',
      'Выбери пластинки для сета': 'Pick records for the set',
      ...
      'Локальное хранение': 'Stored locally',
      'Личная коллекция — на вашем устройстве': 'Your personal collection stays on this device',
      'VERTAX хранит вашу личную коллекцию в этом браузере. Обезличенные данные релизов и треков могут пополнять общий музыкальный каталог — без имени владельца и без привязки к вашему профилю.':
        'VERTAX stores your personal collection in this browser. Anonymous release and track metadata may improve the shared music catalog, without an owner name or a link to your profile.',
      'Добавляй треки из коллекции и собирай порядок вручную.':
        'Add tracks from your collection and arrange them manually.',
    ...
    zh: {
      ...
      'by Михаил Проскурин': 'by Michael Proskurin',
      'Михаил Проскурин': 'Michael Proskurin',
      'Локальное хранение': '本地存储',
      'Личная коллекция — на вашем устройстве': '个人收藏保存在此设备上',
      'VERTAX хранит вашу личную коллекцию в этом браузере. Обезличенные данные релизов и треков могут пополнять общий музыкальный каталог — без имени владельца и без привязки к вашему профилю.':
        'VERTAX 会将您的个人收藏保存在此浏览器中。匿名的发行和曲目元数据可能用于完善共享音乐目录，不包含所有者姓名，也不会关联您的个人资料。',
      'Добавляй треки из коллекции и собирай порядок вручную.': '从收藏中添加曲目并手动调整顺序。',
      'Найди релиз в Discogs текстом, подтяни треклист, проставь BPM/Key, собери сет по правилу «не два трека с одной пластинки подряд».':
    ...
    ja: {
      ...
      'by Михаил Проскурин': 'by Michael Proskurin',
      'Михаил Проскурин': 'Michael Proskurin',
      'Локальное хранение': 'ローカル保存',
      'Личная коллекция — на вашем устройстве': '個人コレクションはこの端末に保存されます',
      'VERTAX хранит вашу личную коллекцию в этом браузере. Обезличенные данные релизов и треков могут пополнять общий музыкальный каталог — без имени владельца и без привязки к вашему профилю.':
        'VERTAX は個人コレクションをこのブラウザに保存します。匿名化されたリリースと曲のメタデータは、所有者名やプロフィールとの紐付けなしで共有カタログの改善に使われることがあります。',
      'Добавляй треки из коллекции и собирай порядок вручную.':
        'コレクションから曲を追加し、順序を手動で整えます。',
      'Найди релиз в Discogs текстом, подтяни треклист, проставь BPM/Key, собери сет по правилу «не два трека с одной пластинки подряд».':
  ...
  function postProcessElements(root, lang, dict) {
    ...
    root.querySelectorAll('[data-action="back"]').forEach(function (el) {
      var translated = dict['Назад'] || dict['← Назад'] || 'Back';
      translated = String(translated).replace(/^←\s*/, '');
      var label = el.querySelector('.laiso-back-label');
