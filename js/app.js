...
(function(){
  if (window.__vertaxLegacyPatchesInstalled) return;
  window.__vertaxLegacyPatchesInstalled = true;
  var queue = (window.__vertaxLegacyPatchQueue || []).slice().sort(function(a,b){ return a.order - b.order; });
  for (var i = 0; i < queue.length; i++) {
    try { queue[i].install(); }
    catch(e) { console.warn('legacy patch failed', queue[i].name, e); }
  }
})();
