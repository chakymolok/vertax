/* VERTAX-01 / RUNT-01 render pipeline and view functions. */

/* ============================================================ */
/* RENDER — split into two independent zones:                   */
/*   #laiso-main    — view content (preserved across toasts)   */
/*   #laiso-overlay — modal + toast (updated independently)    */
/*                                                             */
/* Before this change every toast/modal open called            */
/* root.innerHTML = html which destroyed the entire DOM,       */
/* losing input focus and scroll position.                     */
/* ============================================================ */

function _ensureZones(root) {
  if (!document.getElementById('laiso-main')) {
    root.innerHTML = '<div id="laiso-main"></div><div id="laiso-overlay"></div>';
  }
}

function render() {
  var root = document.getElementById('laiso-root');
  if (!root) return;

  _ensureZones(root);
  var mainEl = document.getElementById('laiso-main');
  var overlayEl = document.getElementById('laiso-overlay');

  // --- Main view (navigation changes) ---
  var viewHtml = '';
  switch (state.view) {
    case 'home':
      viewHtml = viewHome();
      break;
    case 'add':
      viewHtml = viewAdd();
      break;
    case 'match':
      viewHtml = viewMatch();
      break;
    case 'tracklist':
      viewHtml = viewTracklist();
      break;
    case 'edit-track':
      viewHtml = viewEditTrack();
      break;
    case 'fetching':
      viewHtml = viewFetching();
      break;
    case 'set':
      viewHtml = viewSet();
      break;
    case 'collection':
      viewHtml = viewCollection();
      break;
    case 'discogs-import':
      viewHtml = viewDiscogsImport();
      break;
    case 'fit-check':
      viewHtml = viewFitCheck();
      break;
    case 'dig':
      viewHtml = viewDig();
      break;
    case 'backup':
      viewHtml = typeof window.viewBackup === 'function' ? window.viewBackup() : viewHome();
      break;
    case 'about':
      viewHtml = viewAbout();
      break;
    default:
      viewHtml = viewHome();
  }
  if (mainEl) mainEl.innerHTML = viewHtml;

  // --- Overlay (toast + modal — never touches main zone) ---
  if (overlayEl) {
    var overlayHtml = '';
    if (state.modal) overlayHtml += renderModal();
    if (state.toast) overlayHtml += '<div class="laiso-toast">' + esc(state.toast) + '</div>';
    overlayEl.innerHTML = overlayHtml;
  }

  if (document.body) document.body.classList.toggle('vertax-dark', state.view === 'live-set');

  if (typeof vertaxAfterRender === 'function') {
    try {
      vertaxAfterRender();
    } catch (_) {}
  }
}
function renderAttribution() {
  return (
    '<div style="margin-top:24px;padding:12px 0;border-top:1px solid var(--border);text-align:center;font-family:var(--font-mono);font-size:10px;color:var(--text-tertiary);letter-spacing:0.04em;">' +
    'BPM/KEY · POWERED BY ' +
    '<a href="https://www.getsongbpm.com" target="_blank" rel="noopener" style="color:var(--accent-lime-dark);text-decoration:underline;">GetSongBPM</a>' +
    ' AND ' +
    '<a href="https://acousticbrainz.org" target="_blank" rel="noopener" style="color:#3a8fd8;text-decoration:underline;">AcousticBrainz</a>' +
    ' AND ' +
    '<a href="https://open.spotify.com" target="_blank" rel="noopener" style="color:#1db954;text-decoration:underline;">Spotify</a>' +
    '</div>'
  );
}
/* ---------- DEVICE CHASSIS (top brand strip) ---------- */ function renderChassis(rightSlot) {
  return (
    '' +
    '<div class="laiso-chassis">' +
    '<div class="laiso-chassis-row">' +
    '<div class="laiso-brand-wrap">' +
    '<div class="laiso-dotmark" aria-hidden="true"><span></span><span></span><span></span></div>' +
    '<div class="laiso-brand">VERTAX-01<small>by Laiso Buck / Михаил Проскурин</small></div>' +
    '</div>' +
    '<div class="laiso-chassis-side">' +
    (rightSlot || '') +
    renderThemeToggle() +
    '</div>' +
    '</div>' +
    '</div>'
  );
}
function renderCounterLcd() {
  var n = (state.collection || []).length;
  return (
    '<span class="laiso-lcd-mini"><span class="laiso-lcd-mini-dot"></span>' + n + '/1000</span>'
  );
}
function renderThemeToggle() {
  var theme = typeof getVertaxTheme === 'function' ? getVertaxTheme() : 'light';
  var isDark = theme === 'dark';
  return (
    '<button class="vertax-theme-toggle' +
    (isDark ? ' is-dark' : '') +
    '" data-action="theme-toggle" aria-label="Переключить тему" title="' +
    (isDark ? 'Graphite Night' : 'Graphite Ice') +
    '" type="button"><span class="vertax-theme-toggle-track"><span class="vertax-theme-toggle-icon vertax-theme-icon-ice">ICE</span><span class="vertax-theme-toggle-icon vertax-theme-icon-night">NGT</span><span class="vertax-theme-toggle-handle"></span></span></button>'
  );
}
function vertaxPad3(n) {
  var s = String(n || 0);
  while (s.length < 3) s = '0' + s;
  return s;
}
function renderVertaxDisplay() {
  var n = ((state && state.collection) || []).length;
  var sets = ((state && state.sets) || []).length;
  return (
    '<div class="vertax-display" id="vertax-display">' +
    '<div class="vertax-display-top">' +
    '<span class="vertax-display-status"><span class="vertax-display-led"></span>READY</span>' +
    '<span class="vertax-display-clock" id="vertax-clock">--<span class="vertax-display-colon">:</span>--</span>' +
    '</div>' +
    '<div class="vertax-display-title">DIG. PLAY. SHARE.</div>' +
    '<div class="vertax-display-stats">' +
    '<span class="vertax-display-stat"><strong>' +
    vertaxPad3(n) +
    '</strong><small>RECORDS</small></span>' +
    '<span class="vertax-display-stat"><strong>' +
    vertaxPad3(sets) +
    '</strong><small>SETS</small></span>' +
    '<span class="vertax-display-stat"><strong>LOCAL</strong><small>DB</small></span>' +
    '</div>' +
    '</div>'
  );
}
/* ---------- HOME ---------- */ function viewHome() {
  return (
    '' +
    '<div class="laiso-home">' +
    renderChassis() +
    renderVertaxDisplay() +
    '<div class="laiso-mod-label">главное меню</div>' +
    '<div class="laiso-home-grid">' +
    '<button class="laiso-btn laiso-btn-main" data-action="go-add" data-testid="home-add-record">Найти пластинку</button>' +
    '<button class="laiso-btn laiso-btn-secondary" data-action="go-collection" data-testid="home-collection">Коллекция</button>' +
    '<button class="laiso-btn laiso-btn-secondary" data-action="go-set" data-testid="home-set-builder">Собрать сет</button>' +
    '<button class="laiso-btn laiso-btn-secondary" data-action="goto-discogs-import" data-testid="home-discogs-import">Импорт из Discogs</button>' +
    '<button class="laiso-btn laiso-btn-secondary" data-action="open-dig" data-testid="open-dig">Что докопать</button>' +
    '<button class="laiso-btn laiso-btn-secondary vertax-fit-home-cta" data-action="goto-fit-check" data-testid="home-fit-check"><span>Подойдёт ли пластинка?</span><small>проверить релиз по коллекции</small></button>' +
    '</div>' +
    '<div class="laiso-home-stickers">' +
    '<button class="laiso-sticker laiso-sticker-backup" data-action="goto-backup" data-testid="home-backup">' +
    '<span>service</span><strong>Резервная копия</strong><small>сохранить коллекцию</small>' +
    '</button>' +
    '<button class="laiso-sticker laiso-sticker-about" data-action="open-about" data-testid="home-about">' +
    '<span>about</span><strong>О проекте</strong><small>что это за штука</small>' +
    '</button>' +
    '<a class="laiso-sticker laiso-sticker-donate" href="https://www.tbank.ru/cf/1ZddEeAvzU1" target="_blank" rel="noopener">' +
    '<span>support</span><strong>Донат</strong><small>поддержать VERTAX-01</small>' +
    '</a>' +
    '</div>' +
    renderFooter() +
    '</div>'
  );
}
function renderFooter() {
  return (
    '' +
    '<div class="laiso-footer">' +
    '<div>VERTAX-01 · by Laiso Buck / Михаил Проскурин</div>' +
    '<div class="laiso-footer-tg"><a href="https://t.me/michael1994lab" target="_blank" rel="noopener">Telegram автору</a></div>' +
    '<div class="laiso-footer-donate"><a href="https://www.tbank.ru/cf/1ZddEeAvzU1" target="_blank" rel="noopener">Поддержать проект</a></div>' +
    '</div>'
  );
}
function renderHeader(title, opts) {
  opts = opts || {};
  return (
    '' +
    renderChassis(opts.right || '') +
    '<div class="laiso-header">' +
    '<button class="laiso-back" data-action="back">← Назад</button>' +
    '<h1 class="laiso-h1">' +
    esc(title) +
    '</h1>' +
    '<div style="min-width:54px;"></div>' +
    '</div>'
  );
}
/* ---------- ADD VINYL — Discogs search ---------- */ function viewAdd() {
  var vinyls = state.vinyls;
  var canAdd = vinyls.length < SESSION_LIMIT;
  var hasUsable = vinyls.some(function (v) {
    return v.status === 'recognized' || v.status === 'manual';
  });
  var u = state.ui;
  /* SEARCH MODULE — main and only path on this screen */ var searchPanel =
    '<div class="laiso-mod-label">найти пластинку</div>' +
    '<div class="laiso-panel">' +
    '<div class="laiso-search">' +
    '<input class="laiso-input" type="text" id="search-input" data-bind="searchQuery" data-action="search-input" ' +
    'placeholder="Артист, релиз или трек…" autocomplete="off" value="' +
    esc(u.searchQuery || '') +
    '">' +
    '</div>' +
    '<button class="laiso-btn laiso-btn-block" data-action="discogs-search" style="margin-top:10px;">' +
    (u.searchLoading ? '<span class="laiso-spinner"></span> Ищу…' : 'Найти в Discogs') +
    '</button>' +
    (u.searchError
      ? '<div class="laiso-hint laiso-hint-warn" style="margin-top:8px;">' +
        esc(u.searchError) +
        '</div>'
      : '<div class="laiso-hint" style="margin-top:8px;">Discogs · виниловые релизы</div>') +
    renderSearchResults() +
    '</div>';
  /* SESSION LIST MODULE */ var listHtml;
  if (vinyls.length === 0) {
    listHtml =
      '<div class="laiso-empty">' +
      '<strong>Слот пуст</strong>' +
      'Введи артиста, релиз или название трека выше' +
      '</div>';
  } else {
    listHtml = '<div class="laiso-stack-sm">' + vinyls.map(renderVinylCard).join('') + '</div>';
  }
  var limitMsg = !canAdd
    ? '<div class="laiso-hint laiso-hint-warn" style="text-align:center;margin:8px 0;">' +
      'Лимит сессии — ' +
      SESSION_LIMIT +
      '. Нажмите «Готово».</div>'
    : '';
  var listSection =
    '<div class="laiso-mod-label">сессия · ' +
    vinyls.length +
    '/' +
    SESSION_LIMIT +
    '</div>' +
    listHtml +
    limitMsg;
  return (
    renderHeader('Добавить пластинку') +
    searchPanel +
    '<div class="laiso-or">сессия</div>' +
    listSection +
    '<div class="laiso-stack" style="margin-top:14px;">' +
    '<button class="laiso-btn laiso-btn-block" data-action="done-add">Готово → треклисты</button>' +
    '</div>'
  );
}
function viewDiscogsImport() {
  var u = state.ui;
  var loaded = u.discogsImportLoaded && !u.discogsImportLoading;
  var found = (u.discogsImportVinyls || []).length;
  if (u.discogsImportEnrichStage) {
    var stageLabel = u.discogsImportEnrichStage === 'meta' ? 'BPM / Key' : 'треклисты';
    var done = u.discogsImportEnrichDone || 0;
    var total = u.discogsImportEnrichTotal || 0;
    var pct = total ? Math.round((done * 100) / total) : 0;
    return (
      renderHeader('Импорт из Discogs') +
      '<div class="laiso-stack"><div class="laiso-panel"><div class="laiso-mod-label">подгружаю ' +
      esc(stageLabel) +
      '</div><div class="laiso-mono-s" style="margin-top:10px;">' +
      esc(done) +
      ' / ' +
      esc(total) +
      ' (' +
      esc(pct) +
      '%)</div><div style="margin-top:10px;height:6px;background:var(--panel-recess);border-radius:999px;overflow:hidden;"><div style="width:' +
      pct +
      '%;height:100%;background:var(--accent-lime);transition:width .25s;"></div></div><button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="discogs-enrich-cancel" style="margin-top:14px;">Прервать (что успели — уже в коллекции)</button></div></div>'
    );
  }
  var progress = u.discogsImportLoading
    ? '<div class="laiso-panel"><div class="laiso-mono-s">Загружено: ' +
      esc(u.discogsImportProgress || 0) +
      ' пластинок...</div></div>'
    : '';
  var optsBlock = '';
  var result = '';
  if (loaded) {
    var tracksChecked = u.discogsImportFetchTracks ? ' checked' : '';
    var metaChecked = u.discogsImportFetchMeta ? ' checked' : '';
    var metaDisabled = u.discogsImportFetchTracks ? '' : ' disabled';
    optsBlock =
      '<div class="laiso-panel"><div class="laiso-mod-label">опции</div><label class="laiso-check" style="display:flex;gap:10px;align-items:flex-start;margin-top:10px;cursor:pointer;"><input type="checkbox" data-action="toggle-discogs-fetch-tracks"' +
      tracksChecked +
      '><span><strong>Сразу подтянуть треклисты</strong><br><span class="laiso-mono-s" style="color:var(--text-secondary);">≈1 сек на пластинку (Discogs API)</span></span></label><label class="laiso-check" style="display:flex;gap:10px;align-items:flex-start;margin-top:10px;cursor:pointer;opacity:' +
      (metaDisabled ? '.45' : '1') +
      ';"><input type="checkbox" data-action="toggle-discogs-fetch-meta"' +
      metaChecked +
      metaDisabled +
      '><span><strong>И BPM / Key</strong><br><span class="laiso-mono-s" style="color:var(--text-secondary);">медленно: ≈0.4 сек на каждый трек</span></span></label></div>';
    result =
      '<div class="laiso-panel"><p style="margin-top:0;">Найдено пластинок: ' +
      esc(found) +
      '</p><p>Как добавить в коллекцию?</p><div class="laiso-stack"><button class="laiso-btn laiso-btn-block" data-action="discogs-import-replace">Заменить текущую коллекцию</button><button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="discogs-import-merge">Объединить с текущей</button><button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="discogs-import-cancel">Отмена</button></div></div>';
  }
  return (
    renderHeader('Импорт из Discogs') +
    '<div class="laiso-stack"><div class="laiso-panel"><p>Введите ваш Discogs username — и мы загрузим вашу коллекцию автоматически.</p><p>Коллекция должна быть публичной в настройках профиля Discogs.</p></div><div class="laiso-panel"><input class="laiso-input" id="discogs-import-username" data-bind="discogsImportUsername" placeholder="Discogs username" autocomplete="username" value="' +
    esc(u.discogsImportUsername || '') +
    '"><button class="laiso-btn laiso-btn-block" data-action="discogs-import-load" style="margin-top:10px;"' +
    (u.discogsImportLoading ? ' disabled' : '') +
    '>' +
    (u.discogsImportLoading
      ? '<span class="laiso-spinner"></span> Загружаем…'
      : 'Загрузить коллекцию') +
    '</button></div>' +
    progress +
    optsBlock +
    result +
    '</div>'
  );
}
function renderSearchResults() {
  var u = state.ui;
  if (u.searchLoading) {
    return (
      '<div style="text-align:center;padding:20px;"><span class="laiso-spinner"></span> ' +
      '<span class="laiso-mono-s" style="color:var(--text-secondary);margin-left:8px;">Ищу в Discogs…</span></div>'
    );
  }
  var results = u.searchResults || [];
  if (results.length === 0) return '';
  var top = results[0];
  var rest = results.slice(1);
  var topBlock =
    '' +
    '<div class="laiso-mod-label" style="margin-top:14px;">кандидат №1</div>' +
    '<div class="laiso-match-top">' +
    (top.thumb
      ? '<div class="laiso-match-cover" style="margin-bottom:10px;"><img src="' +
        esc(top.thumb) +
        '" alt=""></div>'
      : '') +
    '<div style="font-weight:600;font-size:14px;text-align:center;margin-bottom:2px;">' +
    esc(top.title || '—') +
    '</div>' +
    '<div class="laiso-meta" style="text-align:center;margin-bottom:10px;text-transform:none;letter-spacing:0;font-size:11px;color:var(--text-secondary);font-family:var(--font-sans);">' +
    esc((top.label && top.label[0]) || '') +
    (top.catno ? ' · ' + esc(top.catno) : '') +
    (top.year ? ' · ' + esc(String(top.year)) : '') +
    (top.format ? ' · ' + esc((top.format || []).join(' / ')) : '') +
    '</div>' +
    '<button class="laiso-btn laiso-btn-block" data-action="search-pick" data-id="' +
    esc(String(top.id)) +
    '">Это моя пластинка</button>' +
    '</div>';
  var restBlock = '';
  if (rest.length > 0) {
    restBlock =
      '<div class="laiso-mod-label" style="margin-top:14px;">варианты</div>' +
      '<div class="laiso-stack-sm">' +
      rest
        .map(function (r) {
          var meta =
            (r.label && r.label[0] ? esc(r.label[0]) : '') +
            (r.catno ? ' · ' + esc(r.catno) : '') +
            (r.year ? ' · ' + esc(String(r.year)) : '');
          return (
            '<div class="laiso-search-result" data-action="search-pick" data-id="' +
            esc(String(r.id)) +
            '">' +
            '<div class="laiso-search-result-cover">' +
            (r.thumb ? '<img src="' + esc(r.thumb) + '" alt="">' : '') +
            '</div>' +
            '<div class="laiso-search-result-info">' +
            '<div class="laiso-search-result-title">' +
            esc(r.title || '—') +
            '</div>' +
            '<div class="laiso-search-result-meta">' +
            meta +
            '</div>' +
            '</div>' +
            '<div class="laiso-mono-s" style="color:var(--led-lime-dark);">→</div>' +
            '</div>'
          );
        })
        .join('') +
      '</div>';
  }
  return topBlock + restBlock;
}
function renderVinylCard(v) {
  var coverHtml;
  if (v.userPhoto) coverHtml = '<img src="' + esc(v.userPhoto) + '" alt="">';
  else if (v.coverUrl) coverHtml = '<img src="' + esc(v.coverUrl) + '" alt="">';
  else coverHtml = 'нет фото';
  var statusMap = {
    awaiting: ['laiso-badge-awaiting', 'ОЖИДАЕТ'],
    recognized: ['laiso-badge-recognized', 'НАЙДЕНА'],
    needs_confirmation: ['laiso-badge-confirm', 'ПОДТВЕРДИТЬ'],
    not_found: ['laiso-badge-not-found', 'НЕ НАЙДЕНА'],
    manual: ['laiso-badge-manual', 'ВРУЧНУЮ'],
  };
  var s = statusMap[v.status] || statusMap.awaiting;
  var spinning =
    v.status === 'awaiting' && v._recognizing ? '<span class="laiso-spinner"></span>' : '';
  var canTracklist =
    (v.status === 'recognized' || v.status === 'manual' || v.status === 'needs_confirmation') &&
    v.tracklist &&
    v.tracklist.length;
  var canConfirm =
    v.status === 'needs_confirmation' && state.ui.candidates && state.ui.candidates.length;
  return (
    '' +
    '<div class="laiso-vinyl-card laiso-vinyl-card-clickable" data-action="collection-open-vinyl" data-id="' +
    esc(v.id) +
    '">' +
    '<div class="laiso-vinyl-cover">' +
    coverHtml +
    '</div>' +
    '<div class="laiso-vinyl-info">' +
    '<div class="laiso-vinyl-title">' +
    esc(v.title || '—') +
    '</div>' +
    '<div class="laiso-vinyl-artist">' +
    esc(v.artist || '—') +
    '</div>' +
    '<div class="laiso-vinyl-meta">' +
    esc(v.label || '') +
    (v.catno ? ' · ' + esc(v.catno) : '') +
    (v.year || v.format
      ? '<br>' + esc(v.year || '') + (v.format ? ' · ' + esc(v.format) : '')
      : '') +
    '</div>' +
    '</div>' +
    '<div class="laiso-vinyl-actions">' +
    '<span class="laiso-badge ' +
    s[0] +
    '">' +
    s[1] +
    ' ' +
    spinning +
    '</span>' +
    '<div class="laiso-menu" data-vinyl-id="' +
    esc(v.id) +
    '">' +
    '<button class="laiso-menu-btn" data-action="toggle-menu" data-id="' +
    esc(v.id) +
    '">⋯</button>' +
    '<div class="laiso-menu-list">' +
    '<button data-action="collection-open-vinyl" data-id="' +
    esc(v.id) +
    '">' +
    (canTracklist ? 'Открыть треклист' : 'Открыть пластинку') +
    '</button>' +
    (v.discogsId
      ? '<button data-action="load-release-tracklist" data-id="' +
        esc(v.id) +
        '">Загрузить треклист</button>'
      : '') +
    (canConfirm
      ? '<button data-action="resume-match" data-id="' +
        esc(v.id) +
        '">Подтвердить совпадение…</button>'
      : '') +
    '<button data-action="recognize-one" data-id="' +
    esc(v.id) +
    '">Распознать заново</button>' +
    '<button data-action="manual-edit-vinyl" data-id="' +
    esc(v.id) +
    '">Править детали…</button>' +
    '<button data-action="vinyl-toggle-exclude" data-vid="' +
    esc(v.id) +
    '">' +
    (v.excludeFromSets ? 'Вернуть в сеты' : 'Пропускать в сетах') +
    '</button>' +
    '<button data-action="delete-vinyl" data-id="' +
    esc(v.id) +
    '" class="danger">Удалить</button>' +
    '</div>' +
    '</div>' +
    '</div>' +
    '</div>'
  );
}
/* ---------- ADD MODAL ---------- */ /* Legacy modal — поиск теперь встроен в Add screen, эта модалка не вызывается из UI, */ /* но оставлена пустой на случай старых ссылок. */ function renderAddModal() {
  return (
    '<div class="laiso-modal-bg" data-action="close-modal-bg">' +
    '<div class="laiso-modal" data-stop>' +
    '<h2 class="laiso-modal-title">Поиск перенесён</h2>' +
    '<p>Поиск пластинок теперь встроен прямо в экран «Добавить пластинку».</p>' +
    '<button class="laiso-btn laiso-btn-block" data-action="close-modal" style="margin-top:14px;">OK</button>' +
    '</div>' +
    '</div>'
  );
}
/* ---------- MANUAL VINYL (when not_found / manual edit) ---------- */ function renderManualVinylModal() {
  var v = state.ui.manualVinyl || {};
  return (
    '' +
    '<div class="laiso-modal-bg" data-action="close-modal-bg">' +
    '<div class="laiso-modal" data-stop>' +
    '<div class="laiso-row" style="justify-content:space-between;align-items:center;margin-bottom:8px;">' +
    '<h2 class="laiso-modal-title">Правка пластинки</h2>' +
    '<button class="laiso-back" data-action="close-modal">Закрыть</button>' +
    '</div>' +
    '<div class="laiso-stack">' +
    inputField('Артист', 'mv-artist', v.artist) +
    inputField('Название', 'mv-title', v.title) +
    '<div class="laiso-row">' +
    '<div class="laiso-grow">' +
    inputField('Лейбл', 'mv-label', v.label) +
    '</div>' +
    '<div class="laiso-grow">' +
    inputField('Каталог №', 'mv-catno', v.catno) +
    '</div>' +
    '</div>' +
    '<div class="laiso-row">' +
    '<div class="laiso-grow">' +
    inputField('Год', 'mv-year', v.year) +
    '</div>' +
    '<div class="laiso-grow">' +
    inputField('Формат', 'mv-format', v.format) +
    '</div>' +
    '</div>' +
    '<button class="laiso-btn laiso-btn-block" data-action="save-manual-vinyl">Сохранить</button>' +
    '</div>' +
    '</div>' +
    '</div>'
  );
}
function inputField(label, id, val) {
  return (
    '<div><label class="laiso-label">' +
    esc(label) +
    '</label>' +
    '<input class="laiso-input" id="' +
    esc(id) +
    '" type="text" value="' +
    esc(val || '') +
    '"></div>'
  );
}
/* ---------- ABOUT / HELP / CONFIRM CLEAR ---------- */ function renderAboutModal() {
  return (
    '<div class="laiso-modal-bg" data-action="close-modal-bg"><div class="laiso-modal" data-stop>' +
    '<div class="laiso-row" style="justify-content:space-between;align-items:center;margin-bottom:8px;">' +
    '<h2 class="laiso-modal-title">О VERTAX-01</h2><button class="laiso-back" data-action="close-modal" data-testid="about-modal-close">Закрыть</button></div>' +
    '<div class="laiso-stack">' +
    '<p>Цифровой DJ-компаньон для винила. Добавь до ' +
    SESSION_LIMIT +
    ' пластинок, посмотри треклист, проставь BPM/Key, собери сет по правилу «не два трека с одной пластинки подряд».</p>' +
    '<p style="font-family:var(--font-mono);font-size:11px;color:var(--text-tertiary);">Discogs · GetSongBPM · AcousticBrainz · IndexedDB</p>' +
    '<p>by <strong>Laiso Buck / Михаил Проскурин</strong> · <a href="https://t.me/michael1994lab" target="_blank" rel="noopener">t.me/michael1994lab</a></p>' +
    '<p><a href="https://www.tbank.ru/cf/1ZddEeAvzU1" target="_blank" rel="noopener">Поддержать проект</a></p>' +
    '</div></div></div>'
  );
}
function renderHelpModal() {
  return (
    '<div class="laiso-modal-bg" data-action="close-modal-bg"><div class="laiso-modal" data-stop>' +
    '<div class="laiso-row" style="justify-content:space-between;align-items:center;margin-bottom:8px;">' +
    '<h2 class="laiso-modal-title">Как пользоваться</h2><button class="laiso-back" data-action="close-modal">Закрыть</button></div>' +
    '<div class="laiso-stack" style="font-size:12px;line-height:1.6;">' +
    '<p><strong>1.</strong> «Добавить пластинку» → введи артиста и название → «→».</p>' +
    '<p><strong>2.</strong> Выбери своего кандидата из списка — подтянется треклист.</p>' +
    '<p><strong>3.</strong> Добавь до ' +
    SESSION_LIMIT +
    ' пластинок, нажми «Готово → треклисты».</p>' +
    '<p><strong>4.</strong> Проверь BPM/Key, собери сет.</p>' +
    '<p style="color:var(--text-tertiary);font-size:11px;">Если открыто как file://, часть API заблокируется CORS. Используй https-хост (Tilda) или локальный сервер.</p>' +
    '</div></div></div>'
  );
}
function renderConfirmClearModal() {
  return (
    '<div class="laiso-modal-bg" data-action="close-modal-bg"><div class="laiso-modal" data-stop>' +
    '<h2 class="laiso-modal-title">Точно стереть все данные?</h2>' +
    '<p>Удалит все пластинки, сеты и локальные настройки приложения. Действие необратимо.</p>' +
    '<div class="laiso-row" style="margin-top:16px;">' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="close-modal">Отмена</button>' +
    '<button class="laiso-btn laiso-btn-warning laiso-grow" data-action="clear-data-confirm">Да, стереть всё</button>' +
    '</div>' +
    '</div></div>'
  );
}
function renderModal() {
  if (state.modal && state.modal.type === 'confirm') return renderRuntimeConfirmModal(state.modal);
  if (state.modal && state.modal.type === 'prompt') return renderRuntimePromptModal(state.modal);
  if (state.modal === 'manual-meta') return renderManualMetaModal();
  if (state.modal === 'add-vinyl') return renderAddModal();
  if (state.modal === 'manual-vinyl') return renderManualVinylModal();
  if (state.modal === 'about') return renderAboutModal();
  if (state.modal === 'help') return renderHelpModal();
  if (state.modal === 'confirm-clear') return renderConfirmClearModal();
  return '';
}
function renderRuntimeConfirmModal(modal) {
  return (
    '<div class="laiso-modal-bg" data-action="vertax-modal-cancel"><div class="laiso-modal" data-stop>' +
    '<h2 class="laiso-modal-title">' +
    esc(modal.title || 'Подтвердить действие') +
    '</h2>' +
    '<p style="white-space:pre-line;">' +
    esc(modal.message || '') +
    '</p>' +
    '<div class="laiso-row" style="margin-top:16px;">' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="vertax-modal-cancel">' +
    esc(modal.cancelText || 'Отмена') +
    '</button>' +
    '<button class="laiso-btn laiso-grow" data-action="vertax-modal-confirm">' +
    esc(modal.confirmText || 'OK') +
    '</button>' +
    '</div>' +
    '</div></div>'
  );
}
function renderRuntimePromptModal(modal) {
  return (
    '<div class="laiso-modal-bg" data-action="vertax-modal-cancel"><div class="laiso-modal" data-stop>' +
    '<h2 class="laiso-modal-title">' +
    esc(modal.title || 'Введите значение') +
    '</h2>' +
    '<div class="laiso-stack">' +
    '<label class="laiso-label" for="vertax-prompt-input">' +
    esc(modal.fieldLabel || modal.message || 'Значение') +
    '</label>' +
    '<input class="laiso-input" id="vertax-prompt-input" data-action="vertax-modal-input" type="text" value="' +
    esc(modal.defaultValue || '') +
    '">' +
    (modal.message && modal.fieldLabel
      ? '<p class="laiso-meta" style="white-space:pre-line;">' + esc(modal.message) + '</p>'
      : '') +
    '<div class="laiso-row" style="margin-top:8px;">' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="vertax-modal-cancel">' +
    esc(modal.cancelText || 'Отмена') +
    '</button>' +
    '<button class="laiso-btn laiso-grow" data-action="vertax-modal-confirm">' +
    esc(modal.confirmText || 'OK') +
    '</button>' +
    '</div>' +
    '</div>' +
    '</div></div>'
  );
}
function renderManualMetaModal() {
  var data = (state.ui && state.ui.manualMeta) || {};
  return (
    '<div class="laiso-modal-bg" data-action="close-modal-bg"><div class="laiso-modal" data-stop>' +
    '<div class="laiso-row" style="justify-content:space-between;align-items:center;margin-bottom:8px;">' +
    '<h2 class="laiso-modal-title">BPM / Key</h2><button class="laiso-back" data-action="manual-meta-cancel">Отмена</button></div>' +
    '<div class="laiso-stack">' +
    '<div><label class="laiso-label">BPM</label><input class="laiso-input" id="manual-meta-bpm" inputmode="decimal" type="text" placeholder="174 или пусто" value="' +
    esc(data.bpm || '') +
    '"></div>' +
    '<div><label class="laiso-label">Key / Camelot</label><input class="laiso-input" id="manual-meta-key" type="text" autocapitalize="characters" placeholder="8A, 9B, Am, F#m" value="' +
    esc(data.key || '') +
    '"></div>' +
    (data.error ? '<div class="laiso-hint laiso-hint-warn">' + esc(data.error) + '</div>' : '') +
    '<div class="laiso-row" style="margin-top:8px;">' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="manual-meta-cancel">Отмена</button>' +
    '<button class="laiso-btn laiso-grow" data-action="manual-meta-save">Сохранить</button>' +
    '</div>' +
    '</div></div></div>'
  );
}

/* ---------- MATCH ---------- */ function viewMatch() {
  var cands = state.ui.candidates || [];
  var top = cands[0];
  var rest = cands.slice(1, 3);
  if (!top) {
    return (
      renderHeader('Подтверждение релиза') +
      '<div class="laiso-empty">Нет кандидатов.</div>' +
      '<div class="laiso-row" style="margin-top:16px;">' +
      '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="back">Назад</button>' +
      '<button class="laiso-btn laiso-grow" data-action="match-manual">Ввести вручную</button>' +
      '</div>'
    );
  }
  var vinyl = findVinyl(state.ui.currentVinylId);
  var conf = vinyl ? vinyl.confidence : 75;
  function candSmall(c, idx) {
    return (
      '<div class="laiso-vinyl-card" style="margin-top:8px;">' +
      '<div class="laiso-vinyl-cover">' +
      (c.coverUrl ? '<img src="' + esc(c.coverUrl) + '" alt="">' : 'нет фото') +
      '</div>' +
      '<div class="laiso-vinyl-info">' +
      '<div class="laiso-vinyl-title">' +
      esc(c.title) +
      '</div>' +
      '<div class="laiso-vinyl-artist">' +
      esc(c.artist) +
      '</div>' +
      '<div class="laiso-vinyl-meta">' +
      esc(c.label || '') +
      (c.catno ? ' · ' + esc(c.catno) : '') +
      (c.year ? ' · ' + esc(c.year) : '') +
      '</div>' +
      '</div>' +
      '<div class="laiso-vinyl-actions">' +
      '<span class="laiso-match-pct">' +
      (conf - 15 - idx * 5) +
      '%</span>' +
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="match-pick" data-idx="' +
      (idx + 1) +
      '">Это моя</button>' +
      '</div>' +
      '</div>'
    );
  }
  var trackPreview = (top.tracklist || [])
    .slice(0, 6)
    .map(function (t) {
      return (
        '<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:11px;font-family:var(--font-mono);">' +
        '<span>' +
        esc(t.position) +
        ' · ' +
        esc(t.title) +
        '</span>' +
        '<span style="color:var(--text-tertiary);">' +
        esc(t.duration || '') +
        '</span>' +
        '</div>'
      );
    })
    .join('');
  if ((top.tracklist || []).length > 6)
    trackPreview +=
      '<div style="font-size:10px;color:var(--text-tertiary);font-family:var(--font-mono);text-transform:uppercase;">+' +
      (top.tracklist.length - 6) +
      ' ещё…</div>';
  return (
    renderHeader('Подтверждение релиза') +
    '<div class="laiso-stack">' +
    '<div class="laiso-match-top">' +
    '<div class="laiso-match-cover">' +
    (top.coverUrl ? '<img src="' + esc(top.coverUrl) + '" alt="">' : '') +
    '</div>' +
    '<div style="text-align:center;margin-bottom:8px;">' +
    '<span class="laiso-match-pct">СОВПАДЕНИЕ ' +
    conf +
    '%</span>' +
    '</div>' +
    '<div style="text-align:center;">' +
    '<div class="laiso-h1">' +
    esc(top.title) +
    '</div>' +
    '<div style="color:var(--text-secondary);margin-top:4px;">' +
    esc(top.artist) +
    '</div>' +
    '<div class="laiso-vinyl-meta" style="margin-top:6px;">' +
    esc(top.label || '') +
    (top.catno ? ' · ' + esc(top.catno) : '') +
    (top.year ? ' · ' + esc(top.year) : '') +
    (top.format ? ' · ' + esc(top.format) : '') +
    '</div>' +
    '</div>' +
    '<div style="margin-top:16px;border-top:1px solid var(--border);padding-top:12px;">' +
    trackPreview +
    '</div>' +
    '<button class="laiso-btn laiso-btn-block" style="margin-top:16px;" data-action="match-pick" data-idx="0">Это моя пластинка</button>' +
    '</div>' +
    (rest.length
      ? '<div><div class="laiso-meta" style="margin-bottom:6px;">Другие кандидаты</div>' +
        rest.map(candSmall).join('') +
        '</div>'
      : '') +
    '<div class="laiso-row" style="margin-top:8px;">' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="match-retake">Переснять</button>' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="match-manual">Вручную</button>' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="match-skip">Пропустить</button>' +
    '</div>' +
    '</div>'
  );
}
/* ---------- TRACKLIST ---------- */ function viewTracklist() {
  var v = findVinyl(state.ui.currentVinylId);
  if (!v) {
    return renderHeader('Треклист') + '<div class="laiso-empty">Пластинка не выбрана</div>';
  }
  var sides = ['A', 'B', 'C', 'D', 'E', 'F'];
  var active = state.ui.activeSide;
  if (sides.indexOf(active) < 0) active = sides[0];
  var tabs =
    '<div class="laiso-tabs laiso-side-tabs">' +
    sides
      .map(function (s) {
        return (
          '<button class="laiso-tab ' +
          (s === active ? 'active' : '') +
          '" data-action="side-tab" data-side="' +
          s +
          '">сторона ' +
          s +
          '</button>'
        );
      })
      .join('') +
    '</div>';
  function srcAbbrev(src) {
    if (src === 'getsongbpm') return { label: 'GSB', color: 'var(--accent-lime-dark)' };
    if (src === 'acousticbrainz') return { label: 'ACB', color: '#3a8fd8' };
    if (src === 'manual') return { label: 'РУЧ', color: 'var(--text-tertiary)' };
    if (src === 'admin') return { label: 'ТВОЁ', color: 'var(--accent-lime-dark)' };
    if (src === 'curated') return { label: 'ПРОВ', color: 'var(--accent-lime-dark)' };
    return null;
  }
  var rows =
    (v.tracklist || [])
      .filter(function (t) {
        return t.side === active;
      })
      .map(function (t) {
        var srcBadge = '';
        var sa = srcAbbrev(t.bpmSource);
        if (sa && t.bpm) {
          srcBadge =
            '<span style="display:block;font-family:var(--font-mono);font-size:8px;color:' +
            sa.color +
            ';letter-spacing:0.04em;margin-top:1px;line-height:1;">' +
            sa.label +
            (t.halftimeCorrected ? ' <span style="color:var(--warning);">½×</span>' : '') +
            (t.conflict
              ? ' <span style="color:var(--warning);" title="Sources disagree">!</span>'
              : '') +
            '</span>';
        }
        /* Mini ×2 / ÷2 button for DnB / drum'n'bass scenarios: */ /* - if BPM exists in 70-100 range → show ×2 */ /* - if originalBpm exists (already doubled) → show ÷2 (revert) */ var x2Btn =
          '';
        if (t.originalBpm && t.bpm) {
          x2Btn =
            '<button class="laiso-x2-btn" data-action="bpm-divide-2" data-vid="' +
            esc(v.id) +
            '" data-tid="' +
            esc(t.id) +
            '" title="вернуть ' +
            esc(String(t.originalBpm)) +
            '">÷2</button>';
        } else if (t.bpm && t.bpm >= 60 && t.bpm <= 115) {
          x2Btn =
            '<button class="laiso-x2-btn" data-action="bpm-x2" data-vid="' +
            esc(v.id) +
            '" data-tid="' +
            esc(t.id) +
            '" title="DnB: удвоить BPM">×2</button>';
        }
        var excludedClass = t.excludeFromSets ? ' is-excluded' : '';
        return (
          '<div class="laiso-track' +
          excludedClass +
          '" data-track-id="' +
          esc(t.id) +
          '">' +
          '<span class="laiso-track-pos">' +
          esc(t.position || '') +
          '</span>' +
          '<span class="laiso-track-title" data-action="edit-track" data-id="' +
          esc(t.id) +
          '">' +
          esc(t.title || '—') +
          (t.excludeFromSets
            ? ' <span style="color:var(--text-tertiary);font-size:10px;">[не в сете]</span>'
            : '') +
          '</span>' +
          '<span class="laiso-track-bpm">' +
          (t.bpm ? '<span class="laiso-track-bpm-val">' + t.bpm + '</span>' : '—') +
          srcBadge +
          x2Btn +
          '<button class="laiso-x2-btn" data-action="edit-track" data-id="' +
          esc(t.id) +
          '" title="править вручную">править</button>' +
          '</span>' +
          '<span class="laiso-track-key">' +
          esc(t.key || '—') +
          '</span>' +
          '<span class="laiso-track-cam">' +
          esc(t.camelot || '—') +
          '</span>' +
          '<div class="laiso-menu" data-track-menu="' +
          esc(t.id) +
          '">' +
          '<button class="laiso-menu-btn" data-action="toggle-menu" data-id="trk-' +
          esc(t.id) +
          '">⋯</button>' +
          '<div class="laiso-menu-list">' +
          '<button data-action="edit-track" data-id="' +
          esc(t.id) +
          '">Править BPM/Key</button>' +
          '<button data-action="track-manual-meta" data-vid="' +
          esc(v.id) +
          '" data-tid="' +
          esc(t.id) +
          '">Ввести BPM/Key…</button>' +
          '<button data-action="track-toggle-exclude" data-vid="' +
          esc(v.id) +
          '" data-tid="' +
          esc(t.id) +
          '">' +
          (t.excludeFromSets ? 'Вернуть в сеты' : 'Не использовать в сете') +
          '</button>' +
          '<button data-action="rename-track" data-id="' +
          esc(t.id) +
          '">Переименовать</button>' +
          '<button data-action="change-side" data-id="' +
          esc(t.id) +
          '">Сменить сторону</button>' +
          '<button data-action="delete-track" data-id="' +
          esc(t.id) +
          '" class="danger">Удалить</button>' +
          '</div>' +
          '</div>' +
          '</div>'
        );
      })
      .join('') || '<div class="laiso-empty">На этой стороне треков нет</div>';
  var listHtml = '<div class="laiso-panel" style="padding:0;overflow:hidden;">' + rows + '</div>';
  var hasTracks = !!(v.tracklist && v.tracklist.length);
  var loadTracklistPanel =
    !hasTracks && v.discogsId
      ? '<div class="laiso-panel laiso-tracklist-empty-panel"><div class="laiso-h2">Треклист ещё не загружен</div><p>Можно загрузить треки из Discogs или добавить их вручную.</p><button class="laiso-btn laiso-btn-block" data-action="load-release-tracklist" data-id="' +
        esc(v.id) +
        '">Загрузить треклист из Discogs</button></div>'
      : '';
  var reloadTracklistButton =
    hasTracks && v.discogsId
      ? '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="load-release-tracklist" data-id="' +
        esc(v.id) +
        '">Обновить треклист из Discogs</button>'
      : '';
  return (
    renderHeader('Треклист') +
    '<div class="laiso-panel">' +
    '<div class="laiso-h1">' +
    esc(v.title || '—') +
    '</div>' +
    '<div style="color:var(--text-secondary);margin-top:2px;">' +
    esc(v.artist || '—') +
    '</div>' +
    '<div class="laiso-vinyl-meta" style="margin-top:8px;">' +
    esc(v.label || '') +
    (v.catno ? ' · ' + esc(v.catno) : '') +
    (v.year ? ' · ' + esc(v.year) : '') +
    (v.format ? ' · ' + esc(v.format) : '') +
    '</div>' +
    '</div>' +
    loadTracklistPanel +
    tabs +
    '<div class="laiso-meta" style="display:flex;justify-content:space-between;padding:0 12px 6px;">' +
    '<span>ПОЗ</span><span style="margin-right:auto;margin-left:36px;">НАЗВАНИЕ</span>' +
    '<span style="margin-right:42px;">BPM</span><span style="margin-right:42px;">KEY</span>' +
    '<span class="laiso-track-cam">CAM</span><span style="width:24px;"></span>' +
    '</div>' +
    listHtml +
    '<div class="laiso-stack" style="margin-top:16px;">' +
    reloadTracklistButton +
    '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="add-track-blank">+ Добавить трек</button>' +
    '<button class="laiso-btn laiso-btn-block" data-action="confirm-tracklist">Подтвердить треклист</button>' +
    '</div>'
  );
}
/* ---------- EDIT TRACK ---------- */ function viewEditTrack() {
  var v = findVinyl(state.ui.currentVinylId);
  var t = v && findTrack(v, state.ui.currentTrackId);
  if (!v || !t)
    return renderHeader('Править трек') + '<div class="laiso-empty">Трек не выбран</div>';
  var keyOpts =
    '<option value="">— выбрать —</option>' +
    ALL_KEYS.map(function (k) {
      return (
        '<option value="' +
        esc(k) +
        '"' +
        (t.key === k ? ' selected' : '') +
        '>' +
        esc(k) +
        ' (' +
        KEY_TO_CAMELOT[k] +
        ')</option>'
      );
    }).join('');
  var camOpts =
    '<option value="">— авто —</option>' +
    Object.keys(CAMELOT_TO_KEY)
      .sort(function (a, b) {
        var na = parseInt(a, 10),
          nb = parseInt(b, 10);
        if (na !== nb) return na - nb;
        return a.slice(-1).localeCompare(b.slice(-1));
      })
      .map(function (c) {
        return (
          '<option value="' +
          c +
          '"' +
          (t.camelot === c ? ' selected' : '') +
          '>' +
          c +
          ' (' +
          CAMELOT_TO_KEY[c] +
          ')</option>'
        );
      })
      .join('');
  var conf =
    t.confidence === 'high' ? 5 : t.confidence === 'medium' ? 3 : t.confidence === 'low' ? 1 : 2;
  var confDots = '';
  for (var i = 1; i <= 5; i++)
    confDots += '<span class="laiso-conf-dot ' + (i <= conf ? 'on' : '') + '"></span>';
  var bpmShow = t.bpm || '---';
  var camShow = t.camelot || '--';
  var keyShow = t.key || '--';
  /* ×2 / ÷2 always available in 70-100 BPM range or when track is already doubled */ var x2Btn =
    '';
  if (t.originalBpm && t.bpm) {
    x2Btn =
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="bpm-divide-2" data-vid="' +
      esc(v.id) +
      '" data-tid="' +
      esc(t.id) +
      '">÷2 · вернуть ' +
      esc(String(t.originalBpm)) +
      '</button>';
  } else if (t.bpm && t.bpm >= 60 && t.bpm <= 115) {
    x2Btn =
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="bpm-x2" data-vid="' +
      esc(v.id) +
      '" data-tid="' +
      esc(t.id) +
      '">×2 (DnB)</button>';
  }
  var doubleBtn = x2Btn;
  /* Halftime info row */ var halftimeRow = '';
  if (t.halftimeCorrected && t.originalBpm) {
    halftimeRow =
      '<div class="laiso-halftime-row">' +
      '<div class="laiso-halftime-label">' +
      'исходно ' +
      esc(String(t.originalBpm)) +
      ' ×2' +
      '</div>' +
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="bpm-divide-2" data-vid="' +
      esc(v.id) +
      '" data-tid="' +
      esc(t.id) +
      '">÷2 · вернуть</button>' +
      '</div>';
  }
  /* Conflict warning (low confidence — sources disagree by ≥3 BPM) */ var conflictRow = '';
  if (t.conflict && t.conflict.getsongbpm && t.conflict.acousticbrainz) {
    conflictRow =
      '<div class="laiso-panel" style="background:#fff8e8;border-color:var(--warning);">' +
      '<div style="font-family:var(--font-mono);font-size:11px;color:var(--warning);letter-spacing:0.04em;font-weight:600;margin-bottom:8px;">⚠ Источники расходятся</div>' +
      '<div style="font-size:13px;color:var(--text-secondary);margin-bottom:10px;">Выбери, кому доверяешь:</div>' +
      '<div class="laiso-row" style="gap:8px;flex-wrap:wrap;">' +
      '<button class="laiso-btn laiso-btn-sm" style="background:var(--accent-lime);color:var(--text-primary);border-color:var(--accent-lime-dark);" data-action="use-conflict-bpm" data-src="getsongbpm">' +
      'GSB · ' +
      esc(String(t.conflict.getsongbpm)) +
      ' BPM' +
      (t.conflict.getsongbpmKey ? ' · ' + esc(t.conflict.getsongbpmKey) : '') +
      '</button>' +
      '<button class="laiso-btn laiso-btn-sm" style="background:#3a8fd8;color:#fff;border-color:#2a6ba8;" data-action="use-conflict-bpm" data-src="acousticbrainz">' +
      'ACB · ' +
      esc(String(t.conflict.acousticbrainz)) +
      ' BPM' +
      (t.conflict.acousticbrainzKey ? ' · ' + esc(t.conflict.acousticbrainzKey) : '') +
      '</button>' +
      '</div>' +
      '</div>';
  }
  /* SOURCE toggle (3 sources with brand colors) */ var srcMeta = {
    getsongbpm: {
      label: 'GETSONGBPM',
      bg: 'var(--accent-lime)',
      color: 'var(--text-primary)',
      border: 'var(--accent-lime-dark)',
    },
    acousticbrainz: { label: 'ACOUSTICBRAINZ', bg: '#3a8fd8', color: '#fff', border: '#2a6ba8' },
    manual: {
      label: 'ВРУЧНУЮ',
      bg: 'var(--bg-panel)',
      color: 'var(--text-secondary)',
      border: 'var(--border)',
    },
  };
  var currentSrc = t.bpmSource || 'manual';
  var srcButtons = ['getsongbpm', 'acousticbrainz', 'manual']
    .map(function (src) {
      var m = srcMeta[src];
      var active = currentSrc === src;
      var style = active
        ? 'background:' + m.bg + ';color:' + m.color + ';border-color:' + m.border + ';'
        : 'background:transparent;color:var(--text-secondary);border-color:var(--border);';
      return (
        '<button style="' +
        style +
        '" data-action="src-toggle" data-src="' +
        src +
        '">' +
        m.label +
        '</button>'
      );
    })
    .join('');
  return (
    renderHeader('Править трек') +
    '<div class="laiso-stack">' +
    '<div class="laiso-lcd">' +
    '<div class="laiso-lcd-label">' +
    esc(t.position || '') +
    ' · ' +
    esc(t.title || '—') +
    '</div>' +
    '<div class="laiso-lcd-xl">' +
    bpmShow +
    (t.halftimeCorrected
      ? ' <span style="font-size:0.4em;color:var(--warning);vertical-align:super;">½×</span>'
      : '') +
    '</div>' +
    '<div class="laiso-lcd-row" style="margin-top:12px;">' +
    '<div class="laiso-lcd-cell"><div class="laiso-lcd-label">KEY</div><div class="laiso-lcd-m">' +
    esc(keyShow) +
    '</div></div>' +
    '<div class="laiso-lcd-cell"><div class="laiso-lcd-label">CAMELOT</div><div class="laiso-lcd-m">' +
    esc(camShow) +
    '</div></div>' +
    '</div>' +
    '</div>' +
    conflictRow +
    '<div class="laiso-panel">' +
    '<label class="laiso-label">BPM</label>' +
    '<div class="laiso-row" style="align-items:center;flex-wrap:wrap;">' +
    '<div class="laiso-stepper">' +
    '<button data-action="bpm-step" data-d="-1">−</button>' +
    '<input id="et-bpm" type="number" min="40" max="240" value="' +
    (t.bpm || '') +
    '" data-action="bpm-input">' +
    '<button data-action="bpm-step" data-d="1">+</button>' +
    '</div>' +
    doubleBtn +
    '</div>' +
    halftimeRow +
    '</div>' +
    '<div class="laiso-panel">' +
    '<label class="laiso-label">Тональность (KEY)</label>' +
    '<select class="laiso-select" id="et-key" data-action="key-change">' +
    keyOpts +
    '</select>' +
    '</div>' +
    '<div class="laiso-panel">' +
    '<label class="laiso-label">CAMELOT (переопределить)</label>' +
    '<select class="laiso-select" id="et-cam" data-action="cam-change">' +
    camOpts +
    '</select>' +
    '</div>' +
    '<div class="laiso-panel">' +
    '<label class="laiso-label">Источник</label>' +
    '<div class="laiso-toggle">' +
    srcButtons +
    '</div>' +
    '<label class="laiso-label" style="margin-top:12px;">Уверенность</label>' +
    '<div class="laiso-conf">' +
    confDots +
    '<span class="laiso-mono-s" style="margin-left:8px;">' +
    esc((t.confidence || 'manual').toUpperCase()) +
    '</span></div>' +
    '</div>' +
    '<div class="laiso-panel">' +
    '<label class="laiso-label">Комментарий</label>' +
    '<textarea class="laiso-textarea" id="et-comment" placeholder="Заметки к миксу…">' +
    esc(t.comment || '') +
    '</textarea>' +
    '</div>' +
    '<div class="laiso-row">' +
    '<button class="laiso-btn laiso-btn-warning" data-action="track-delete">Удалить трек</button>' +
    '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="track-cancel">Отмена</button>' +
    '<button class="laiso-btn laiso-grow" data-action="track-save">Сохранить</button>' +
    '</div>' +
    '</div>'
  );
}
/* ---------- FETCHING METADATA ---------- */ function fetchSafeEsc(s) {
  if (typeof esc === 'function') return esc(s);
  if (s == null) return '';
  return String(s).replace(/[&<>"']/g, function (c) {
    return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c];
  });
}
function findVinylForFetchItem(item) {
  var fp = state.ui && state.ui.fetchProgress;
  if (!fp) return null;
  if (typeof findVinyl !== 'function') return null;
  return findVinyl(item.vinylId || fp.vinylId);
}
function findTrackForFetchItem(item) {
  var v = findVinylForFetchItem(item);
  if (!v || typeof findTrack !== 'function') return null;
  return findTrack(v, item.trackId);
}
function normalizeFetchItemFromTrack(item) {
  var v = findVinylForFetchItem(item);
  var t = findTrackForFetchItem(item);
  if (!t) return item;
  item.title = t.title || item.title;
  item.artist = (v && v.artist) || item.artist;
  if (t.excludeFromSets) {
    item.status = 'skipped';
    item.meta = null;
    return item;
  }
  if (t.bpm || t.camelot || t.key) {
    item.status = 'ok';
    item.meta = {
      bpm: t.bpm || null,
      key: t.key || null,
      camelot: t.camelot || null,
      source: t.bpmSource || t.keySource || 'manual',
      confidence: t.confidence || 'manual',
    };
    item.manualUpdated = true;
  }
  return item;
}
function sourceLabel(src) {
  if (src === 'getsongbpm') return 'GETSONGBPM';
  if (src === 'acousticbrainz') return 'ACOUSTICBRAINZ';
  if (src && src.indexOf('manual') >= 0) return 'ВРУЧНУЮ';
  return src ? String(src).toUpperCase() : 'ВРУЧНУЮ';
}
function sourceColor(src) {
  if (src === 'getsongbpm') return 'var(--accent-lime-dark)';
  if (src === 'acousticbrainz') return '#3a8fd8';
  if (src && src.indexOf('manual') >= 0) return 'var(--warning)';
  return 'var(--text-tertiary)';
}
function renderFetchingRow(item) {
  item = normalizeFetchItemFromTrack(item);
  var v = findVinylForFetchItem(item);
  var t = findTrackForFetchItem(item);
  var vid =
    (v && v.id) || item.vinylId || (state.ui.fetchProgress && state.ui.fetchProgress.vinylId) || '';
  var tid = item.trackId || (t && t.id) || '';
  var title = (t && t.title) || item.title || '—';
  var artist = (v && v.artist) || item.artist || '—';
  var status = item.status || 'pending';
  var meta = item.meta || {};
  var hasValues = !!(meta.bpm || meta.camelot || meta.key || (t && (t.bpm || t.camelot || t.key)));
  var icon = '·';
  var iconColor = 'var(--text-tertiary)';
  var right = '';
  if (status === 'ok' && hasValues) {
    icon = '✓';
    iconColor = 'var(--accent-lime-dark)';
    var bpm = meta.bpm || (t && t.bpm) || '—';
    var cam = meta.camelot || (t && t.camelot) || '—';
    var key = meta.key || (t && t.key) || null;
    var src = meta.source || (t && (t.bpmSource || t.keySource)) || 'manual';
    right =
      '<div class="runt-fetch-values">' +
      '<span class="runt-fetch-pill runt-fetch-bpm">' +
      fetchSafeEsc(bpm) +
      ' BPM</span>' +
      (cam && cam !== '—'
        ? '<span class="runt-fetch-pill runt-fetch-cam">' + fetchSafeEsc(cam) + '</span>'
        : '') +
      (key ? '<span class="runt-fetch-pill runt-fetch-key">' + fetchSafeEsc(key) + '</span>' : '') +
      '</div>' +
      '<div class="runt-fetch-source" style="color:' +
      sourceColor(src) +
      ';">' +
      sourceLabel(src) +
      ' · можно изменить</div>';
  } else if (status === 'fetching') {
    icon = '…';
    right =
      '<span class="laiso-spinner"></span> <span class="laiso-mono-s" style="color:var(--text-secondary);">ИЩЕМ…</span>';
  } else if (status === 'skipped') {
    icon = '⏭';
    right = '<span class="laiso-mono-s" style="color:var(--text-tertiary);">НЕ В СЕТЕ</span>';
  } else if (status === 'notfound' || status === 'error') {
    icon = '×';
    iconColor = 'var(--warning)';
    right = '<span class="laiso-mono-s" style="color:var(--text-tertiary);">НЕ НАЙДЕНО</span>';
  } else {
    right = '<span class="laiso-mono-s" style="color:var(--text-tertiary);">В ОЧЕРЕДИ</span>';
  }
  var actionButtons = '';
  if (status === 'skipped') {
    actionButtons =
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="track-toggle-exclude" data-vid="' +
      fetchSafeEsc(vid) +
      '" data-tid="' +
      fetchSafeEsc(tid) +
      '">Вернуть в сет</button>';
  } else {
    actionButtons =
      '<button class="laiso-btn laiso-btn-sm" data-action="track-manual-meta" data-vid="' +
      fetchSafeEsc(vid) +
      '" data-tid="' +
      fetchSafeEsc(tid) +
      '">' +
      (hasValues ? 'Изменить BPM/Key' : 'Ввести BPM/Key') +
      '</button>' +
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="track-toggle-exclude" data-vid="' +
      fetchSafeEsc(vid) +
      '" data-tid="' +
      fetchSafeEsc(tid) +
      '">Не в сете</button>';
  }
  return (
    '' +
    '<div class="runt-fetch-row">' +
    '<div class="runt-fetch-icon" style="color:' +
    iconColor +
    ';">' +
    icon +
    '</div>' +
    '<div class="runt-fetch-main">' +
    '<div class="runt-fetch-title">' +
    fetchSafeEsc(title) +
    '</div>' +
    '<div class="runt-fetch-artist">' +
    fetchSafeEsc(artist) +
    '</div>' +
    '</div>' +
    '<div class="runt-fetch-status">' +
    right +
    '</div>' +
    '<div class="runt-fetch-actions">' +
    actionButtons +
    '</div>' +
    '</div>'
  );
}
function viewFetching() {
  var fp = state.ui.fetchProgress || {};
  var items = fp.items || [];
  var done = fp.doneCount || 0;
  var total = fp.total || 0;
  var pct = total ? Math.round((done * 100) / total) : 0;
  var complete = !!fp.complete || (total > 0 && done >= total);
  var notFoundCount = items.filter(function (x) {
    var t = findTrackForFetchItem(x);
    return (
      (x.status === 'notfound' || x.status === 'error') &&
      !(t && (t.bpm || t.camelot || t.key || t.excludeFromSets))
    );
  }).length;
  var rows = items.length
    ? items.map(renderFetchingRow).join('')
    : '<div class="laiso-empty">В очереди ничего нет</div>';
  return (
    renderHeader('Подгружаем метаданные') +
    '<div class="laiso-stack">' +
    '<div class="laiso-panel">' +
    '<div style="margin-bottom:10px;font-size:13px;">Ищем BPM и тональность для треков…</div>' +
    '<div class="laiso-meta" style="margin-bottom:6px;">ПРОГРЕСС · ' +
    done +
    ' / ' +
    total +
    ' · ' +
    pct +
    '%</div>' +
    '<div class="runt-fetch-progress"><div style="width:' +
    pct +
    '%;"></div></div>' +
    (notFoundCount > 0
      ? '<div class="runt-fetch-note">По ' +
        notFoundCount +
        ' трек./трекам данные не нашлись. Это нормально: можно нажать «Ввести BPM/Key», потом снова нажать «Изменить BPM/Key», если ошибся.</div>'
      : '') +
    '</div>' +
    '<div class="laiso-panel runt-fetch-panel">' +
    rows +
    '</div>' +
    '<button class="laiso-btn laiso-btn-block" data-action="fetching-done">Дальше → треклист</button>' +
    (!complete
      ? '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="skip-remaining">Пропустить остаток — править вручную</button>'
      : '') +
    '</div>'
  );
}
/* ---------- SET CONSTRUCTOR ---------- */ function viewSet() {
  var mode = state.ui.setMode;
  var opts = state.ui.setOptions;
  var hasSet = state.ui.generatedSet && state.ui.generatedSet.length > 0;
  var modeTabs =
    '<div class="laiso-tabs laiso-tabs-modes">' +
    [
      ['best-flow', 'BEST FLOW'],
      ['tempo-safe', 'ПО ТЕМПУ'],
      ['camelot-safe', 'ПО CAMELOT'],
      ['camelot-filter', 'ФИЛЬТР CAMELOT'],
    ]
      .map(function (p) {
        return (
          '<button class="laiso-tab ' +
          (mode === p[0] ? 'active' : '') +
          '" data-action="set-mode" data-mode="' +
          p[0] +
          '">' +
          p[1] +
          '</button>'
        );
      })
      .join('') +
    '</div>';
  var modeHints = {
    'best-flow': 'Главный режим: BPM-плавность + гармоничный Camelot + правило винила.',
    'tempo-safe': 'Только плавный темп. Каждый следующий трек в пределах ± BPM от предыдущего.',
    'camelot-safe': 'Гармоничный микс: соседние Camelot-коды. Темп не больше ±6 BPM.',
    'camelot-filter':
      'Собрать сет только из выбранных Camelot-кодов. Нажми на круг, чтобы добавить/убрать.',
  };
  var controls =
    '<div class="laiso-mod-label">режим</div>' +
    '<div class="laiso-panel laiso-mode-hint">' +
    esc(modeHints[mode] || '') +
    '</div>';
  if (mode === 'tempo-safe' || mode === 'best-flow') {
    controls +=
      '<div class="laiso-panel">' +
      '<label class="laiso-label">Диапазон темпа ± ' +
      opts.tempoRange +
      ' BPM</label>' +
      '<div class="laiso-toggle">' +
      [2, 4, 6]
        .map(function (n) {
          return (
            '<button class="' +
            (opts.tempoRange === n ? 'active' : '') +
            '" data-action="set-tempo" data-n="' +
            n +
            '">±' +
            n +
            '</button>'
          );
        })
        .join('') +
      '</div>' +
      '</div>';
  } else if (mode === 'camelot-safe') {
    controls +=
      '<div class="laiso-panel"><label class="laiso-label">Стартовый Camelot (опционально)</label>' +
      renderCamelotWheel(opts.targetCamelot) +
      '<div class="laiso-meta" style="text-align:center;margin-top:6px;">' +
      (opts.targetCamelot ? esc(opts.targetCamelot) : 'выбери код или оставь пустым') +
      '</div></div>';
  } else if (mode === 'camelot-filter') {
    var sel = opts.camelotSet || {};
    var selKeys = Object.keys(sel).filter(function (k) {
      return sel[k];
    });
    controls +=
      '<div class="laiso-panel">' +
      '<label class="laiso-label">Выбери коды (' +
      selKeys.length +
      ' выбрано)</label>' +
      renderCamelotWheelMulti(sel) +
      '<div class="laiso-row" style="margin-top:10px;">' +
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary laiso-grow" data-action="camelot-filter-clear">Сбросить</button>' +
      '<button class="laiso-btn laiso-btn-sm laiso-grow" data-action="camelot-filter-apply"' +
      (selKeys.length === 0 ? ' disabled' : '') +
      '>Собрать из выбранных</button>' +
      '</div>' +
      '</div>';
  }
  /* Big-card setlist render */ var setHtml = '';
  if (hasSet) {
    var rows = state.ui.generatedSet
      .map(function (tr, idx, arr) {
        var prev = idx > 0 ? arr[idx - 1] : null;
        var dBpm = prev && prev.bpm && tr.bpm ? tr.bpm - prev.bpm : null;
        var sameVinyl = prev && prev.recordId === tr.recordId;
        var pos = tr.displayPosition || tr.position || '—';
        var bpmHint = '';
        if (tr.originalBpm && tr.bpm) {
          bpmHint =
            '<span class="laiso-set-bpm-hint">исх. ' + esc(String(tr.originalBpm)) + ' ×2</span>';
        }
        /* Inline ×2 / ÷2 button (small) */ var x2 = '';
        if (tr.originalBpm && tr.bpm) {
          x2 =
            '<button class="laiso-x2-btn" data-action="bpm-divide-2" data-vid="' +
            esc(tr.recordId) +
            '" data-tid="' +
            esc(tr.id) +
            '" title="вернуть исходный">÷2</button>';
        } else if (tr.bpm && tr.bpm >= 70 && tr.bpm <= 100) {
          x2 =
            '<button class="laiso-x2-btn" data-action="bpm-x2" data-vid="' +
            esc(tr.recordId) +
            '" data-tid="' +
            esc(tr.id) +
            '" title="DnB ×2">×2</button>';
        }
        var warning = sameVinyl
          ? '<div class="laiso-warn-line">⚠ Та же пластинка, что предыдущий трек — переставь</div>'
          : '';
        var dStr = dBpm === null ? '' : ' (' + (dBpm > 0 ? '+' : '') + dBpm + ')';
        var catnoStr = tr.vinylCatno ? ' · ' + esc(tr.vinylCatno) : '';
        return (
          warning +
          '<div class="laiso-set-card">' +
          '<div class="laiso-set-card-head">' +
          '<span class="laiso-set-pos">[' +
          esc(pos) +
          ']</span>' +
          '<span class="laiso-set-num">#' +
          (idx + 1) +
          '</span>' +
          '<span class="laiso-set-title">' +
          esc(tr.title || '—') +
          '</span>' +
          '</div>' +
          '<div class="laiso-set-meta">' +
          esc(tr.vinylArtist || '') +
          ' — ' +
          esc(tr.vinylTitle || '') +
          catnoStr +
          '</div>' +
          '<div class="laiso-set-pills">' +
          '<span class="laiso-pill laiso-pill-bpm">' +
          (tr.bpm ? tr.bpm + ' BPM' + dStr : '— BPM') +
          '</span>' +
          (tr.key ? '<span class="laiso-pill">Key: ' + esc(tr.key) + '</span>' : '') +
          (tr.camelot
            ? '<span class="laiso-pill laiso-pill-cam">' + esc(tr.camelot) + '</span>'
            : '') +
          x2 +
          bpmHint +
          '</div>' +
          '</div>'
        );
      })
      .join('');
    setHtml =
      '<div class="laiso-mod-label">сет · ' +
      state.ui.generatedSet.length +
      ' трек.</div>' +
      '<div class="laiso-stack-sm">' +
      rows +
      '</div>';
  }
  /* Insufficient-data diagnostics */ var allTracks = getAllSessionTracks({ includeAll: true });
  var availTracks = getAllSessionTracks();
  var availCount = availTracks.length;
  var withBpm = availTracks.filter(function (t) {
    return t.bpm;
  }).length;
  var withCam = availTracks.filter(function (t) {
    return t.camelot;
  }).length;
  var dataPanel = '';
  if (
    state.ui.setOpenDataPanel ||
    state.ui.setLastWarning === 'not-enough-data' ||
    state.ui.setLastWarning === 'no-valid-set'
  ) {
    var problemTracks = allTracks.filter(function (t) {
      if (mode === 'camelot-safe') return !t.camelot;
      if (mode === 'tempo-safe' || mode === 'best-flow') return !t.bpm;
      return false;
    });
    var rowsProblem =
      problemTracks
        .slice(0, 30)
        .map(function (t) {
          var pos = t.displayPosition || '—';
          return (
            '<div class="laiso-data-row">' +
            '<div class="laiso-data-row-info">' +
            '<span class="laiso-set-pos laiso-set-pos-sm">[' +
            esc(pos) +
            ']</span>' +
            '<span>' +
            esc(t.title || '—') +
            '<small> · ' +
            esc(t.vinylArtist || '') +
            ' — ' +
            esc(t.vinylTitle || '') +
            '</small></span>' +
            '</div>' +
            '<div class="laiso-data-row-meta">' +
            (t.bpm
              ? '<span class="laiso-pill">' + t.bpm + ' BPM</span>'
              : '<span class="laiso-pill laiso-pill-warn">нет BPM</span>') +
            (t.camelot
              ? '<span class="laiso-pill">' + esc(t.camelot) + '</span>'
              : '<span class="laiso-pill laiso-pill-warn">нет Camelot</span>') +
            '</div>' +
            '<div class="laiso-data-row-actions">' +
            '<button class="laiso-btn laiso-btn-sm" data-action="track-manual-meta" data-vid="' +
            esc(t.recordId) +
            '" data-tid="' +
            esc(t.id) +
            '">Ввести BPM/Key</button>' +
            '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="track-toggle-exclude" data-vid="' +
            esc(t.recordId) +
            '" data-tid="' +
            esc(t.id) +
            '">' +
            (t.trackExcluded ? 'Вернуть' : 'Не в сете') +
            '</button>' +
            '</div>' +
            '</div>'
          );
        })
        .join('') ||
      '<div class="laiso-empty laiso-empty-sm">В этом режиме все треки укладываются.</div>';
    var warnHeader =
      state.ui.setLastWarning === 'no-valid-set'
        ? 'Не удалось собрать сет в этом режиме'
        : 'Недостаточно данных для сета';
    dataPanel =
      '<div class="laiso-mod-label">диагностика</div>' +
      '<div class="laiso-panel laiso-data-panel">' +
      '<div class="laiso-data-head">' +
      '<strong>' +
      esc(warnHeader) +
      '</strong>' +
      '<span class="laiso-meta">всего: ' +
      availCount +
      ' · с BPM: ' +
      withBpm +
      ' · с Camelot: ' +
      withCam +
      '</span>' +
      '</div>' +
      '<p class="laiso-data-hint">Заполни BPM/Key вручную, исключи трек или пластинку из сетов, или попробуй другой режим.</p>' +
      '<div class="laiso-stack-sm">' +
      rowsProblem +
      '</div>' +
      '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary laiso-btn-block" style="margin-top:10px;" data-action="toggle-data-panel">Скрыть диагностику</button>' +
      '</div>';
  }
  var openDataLink =
    !state.ui.setOpenDataPanel && availCount >= 2
      ? '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary laiso-btn-block" data-action="toggle-data-panel" style="margin-top:8px;">Показать диагностику данных</button>'
      : '';
  return (
    renderHeader('Сборка сета', {
      right: '<span class="laiso-counter">' + availCount + ' трек.</span>',
    }) +
    modeTabs +
    controls +
    '<div class="laiso-row" style="margin:14px 0;">' +
    '<button class="laiso-btn laiso-grow" data-action="set-generate">' +
    (hasSet ? 'Пересобрать' : 'Сгенерировать') +
    '</button>' +
    '</div>' +
    (availCount < 2
      ? '<div class="laiso-empty">Нужно минимум 2 трека. Добавь пластинки и проставь BPM/Key.</div>'
      : '') +
    dataPanel +
    setHtml +
    (hasSet
      ? '<div class="laiso-row" style="margin-top:14px;">' +
        '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="set-export">Экспорт TXT</button>' +
        '<button class="laiso-btn laiso-grow" data-action="set-save">Сохранить сет</button>' +
        '</div>'
      : '') +
    openDataLink
  );
}
/* Multi-select Camelot wheel: opts is { '8A': true, ... } */ function renderCamelotWheelMulti(
  set
) {
  var cx = 150,
    cy = 150;
  var rOuter = 140,
    rMid = 95,
    rInner = 50;
  var sectors = '';
  var labels = '';
  for (var i = 0; i < 12; i++) {
    var num = ((i + 11) % 12) + 1;
    var aStart = (i - 3) * 30;
    var aEnd = aStart + 30;
    sectors += sectorPath(
      cx,
      cy,
      rMid,
      rOuter,
      aStart,
      aEnd,
      'B' + num,
      num + 'B',
      set[num + 'B'] ? num + 'B' : null
    );
    sectors += sectorPath(
      cx,
      cy,
      rInner,
      rMid,
      aStart,
      aEnd,
      'A' + num,
      num + 'A',
      set[num + 'A'] ? num + 'A' : null
    );
    var midA = (aStart + aEnd) / 2;
    var rad = (midA * Math.PI) / 180;
    var lx1 = cx + (Math.cos(rad) * (rOuter + rMid)) / 2;
    var ly1 = cy + (Math.sin(rad) * (rOuter + rMid)) / 2;
    var lx2 = cx + (Math.cos(rad) * (rInner + rMid)) / 2;
    var ly2 = cy + (Math.sin(rad) * (rInner + rMid)) / 2;
    labels +=
      '<text class="laiso-wheel-label ' +
      (set[num + 'B'] ? 'active' : '') +
      '" x="' +
      lx1.toFixed(1) +
      '" y="' +
      (ly1 + 3).toFixed(1) +
      '" text-anchor="middle">' +
      num +
      'B</text>';
    labels +=
      '<text class="laiso-wheel-label ' +
      (set[num + 'A'] ? 'active' : '') +
      '" x="' +
      lx2.toFixed(1) +
      '" y="' +
      (ly2 + 3).toFixed(1) +
      '" text-anchor="middle">' +
      num +
      'A</text>';
  }
  return (
    '<div class="laiso-wheel-wrap"><svg viewBox="0 0 300 300" class="laiso-wheel" xmlns="http://www.w3.org/2000/svg">' +
    sectors +
    labels +
    '</svg></div>'
  );
}
/* Camelot wheel SVG */ function renderCamelotWheel(active) {
  var cx = 150,
    cy = 150;
  var rOuter = 140,
    rMid = 95,
    rInner = 50;
  var sectors = '';
  var labels = '';
  for (var i = 0; i < 12; i++) {
    var num = ((i + 11) % 12) + 1;
    /* 1..12 starting from top */ var aStart = (i - 3) * 30;
    /* top-aligned */ var aEnd = aStart + 30;
    sectors += sectorPath(cx, cy, rMid, rOuter, aStart, aEnd, 'B' + num, num + 'B', active);
    sectors += sectorPath(cx, cy, rInner, rMid, aStart, aEnd, 'A' + num, num + 'A', active);
    /* labels */ var midA = (aStart + aEnd) / 2;
    var rad = (midA * Math.PI) / 180;
    var lx1 = cx + (Math.cos(rad) * (rOuter + rMid)) / 2;
    var ly1 = cy + (Math.sin(rad) * (rOuter + rMid)) / 2;
    var lx2 = cx + (Math.cos(rad) * (rInner + rMid)) / 2;
    var ly2 = cy + (Math.sin(rad) * (rInner + rMid)) / 2;
    labels +=
      '<text class="laiso-wheel-label ' +
      (active === num + 'B' ? 'active' : '') +
      '" x="' +
      lx1.toFixed(1) +
      '" y="' +
      (ly1 + 3).toFixed(1) +
      '" text-anchor="middle">' +
      num +
      'B</text>';
    labels +=
      '<text class="laiso-wheel-label ' +
      (active === num + 'A' ? 'active' : '') +
      '" x="' +
      lx2.toFixed(1) +
      '" y="' +
      (ly2 + 3).toFixed(1) +
      '" text-anchor="middle">' +
      num +
      'A</text>';
  }
  return (
    '<div class="laiso-wheel-wrap"><svg class="laiso-wheel" viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">' +
    sectors +
    labels +
    '</svg></div>'
  );
}
function sectorPath(cx, cy, r1, r2, aStart, aEnd, label, code, active) {
  var rs = (aStart * Math.PI) / 180,
    re = (aEnd * Math.PI) / 180;
  var x1 = cx + r2 * Math.cos(rs),
    y1 = cy + r2 * Math.sin(rs);
  var x2 = cx + r2 * Math.cos(re),
    y2 = cy + r2 * Math.sin(re);
  var x3 = cx + r1 * Math.cos(re),
    y3 = cy + r1 * Math.sin(re);
  var x4 = cx + r1 * Math.cos(rs),
    y4 = cy + r1 * Math.sin(rs);
  var d =
    'M' +
    x1.toFixed(1) +
    ',' +
    y1.toFixed(1) +
    ' A' +
    r2 +
    ',' +
    r2 +
    ' 0 0 1 ' +
    x2.toFixed(1) +
    ',' +
    y2.toFixed(1) +
    ' L' +
    x3.toFixed(1) +
    ',' +
    y3.toFixed(1) +
    ' A' +
    r1 +
    ',' +
    r1 +
    ' 0 0 0 ' +
    x4.toFixed(1) +
    ',' +
    y4.toFixed(1) +
    ' Z';
  var fill = code.endsWith('B') ? '#FAFAF7' : '#F5F2ED';
  var cls = 'laiso-wheel-sec' + (active === code ? ' active' : '');
  return (
    '<path class="' +
    cls +
    '" d="' +
    d +
    '" fill="' +
    fill +
    '" stroke="#D4D0C8" stroke-width="0.5" data-action="wheel-pick" data-code="' +
    code +
    '"></path>'
  );
}
function renderFitCheckCandidate(c) {
  return (
    '<button class="laiso-vinyl-card" data-action="fit-check-candidate" data-release-id="' +
    esc(c.discogs_id) +
    '" style="width:100%;text-align:left;">' +
    '<div class="laiso-vinyl-cover" style="font-size:10px;">' +
    (c.cover_url || c.coverUrl
      ? '<img src="' + esc(c.cover_url || c.coverUrl) + '" alt="">'
      : esc(String(c.year || ''))) +
    '</div>' +
    '<div class="laiso-vinyl-info"><div class="laiso-vinyl-title">' +
    esc(c.title || '—') +
    '</div><div class="laiso-vinyl-artist">' +
    esc([c.label, c.catalog_number, c.country].filter(Boolean).join(' · ')) +
    '</div><div class="laiso-vinyl-meta">' +
    esc(c.format || '') +
    '</div></div><div class="laiso-vinyl-actions"><span class="laiso-badge laiso-badge-awaiting">выбрать</span></div></button>'
  );
}
function formatPercentValue(value) {
  return Math.round((Number(value) || 0) * 100) + '%';
}
function renderFitCheckAutoSummary(text) {
  var raw = String(text || '').trim();
  if (!raw) return '';
  var chunks = raw
    .split('. ')
    .map(function (part, idx, arr) {
      var s = part.trim();
      if (!s) return '';
      if (idx < arr.length - 1 && !/[.!?]$/.test(s)) s += '.';
      return s;
    })
    .filter(Boolean)
    .slice(0, 5);
  return (
    '<div class="vertax-fit-auto-summary"><strong>Коротко</strong>' +
    chunks
      .map(function (part) {
        return '<p>' + esc(part) + '</p>';
      })
      .join('') +
    '</div>'
  );
}
function renderFitCheckAiText(text) {
  var raw = String(text || '').trim();
  if (!raw) return '';
  var parts = raw
    .split(/\n{2,}/)
    .map(function (part) {
      return part.trim();
    })
    .filter(Boolean);
  if (!parts.length) parts = [raw];
  return parts
    .slice(0, 4)
    .map(function (part) {
      return '<p>' + esc(part) + '</p>';
    })
    .join('');
}
function renderFitCheckMatch(match) {
  var rt = match.release_track || {};
  var best = (match.best_collection_matches && match.best_collection_matches[0]) || {};
  var releaseMeta = [rt.bpm ? rt.bpm + ' BPM' : '— BPM', rt.camelot || '—'].join(' / ');
  var collectionTitle = [best.artist, best.title || '—'].filter(Boolean).join(' — ');
  var reasons = (best.reasons || []).join(', ');
  return (
    '<article class="vertax-fit-match-card">' +
    '<div class="vertax-fit-match-top">' +
    '<div class="vertax-fit-track-title"><span class="vertax-fit-pos">' +
    esc(rt.position || '—') +
    '</span><div><strong>' +
    esc(rt.title || '—') +
    '</strong><small>' +
    esc(releaseMeta) +
    '</small></div></div>' +
    '<div class="vertax-fit-score"><strong>' +
    formatPercentValue(best.compatibility) +
    '</strong><span>match</span></div></div>' +
    '<div class="vertax-fit-link"><span>→</span><strong>' +
    esc(collectionTitle) +
    '</strong></div>' +
    '<div class="vertax-fit-chips"><span class="vertax-fit-chip">' +
    esc(best.bpm ? best.bpm + ' BPM' : '— BPM') +
    '</span>' +
    (best.camelot
      ? '<span class="vertax-fit-chip vertax-fit-chip-key">' + esc(best.camelot) + '</span>'
      : '') +
    '<span class="vertax-fit-chip vertax-fit-chip-score">' +
    formatPercentValue(best.compatibility) +
    '</span></div>' +
    (reasons
      ? '<p class="vertax-fit-reasons"><strong>Причина:</strong> ' + esc(reasons) + '</p>'
      : '') +
    '</article>'
  );
}
function renderFitCheckManualTrack(track, idx) {
  var key = esc(track.position || String(idx));
  return (
    '<div class="laiso-panel"><div class="laiso-h2">' +
    esc([track.position, track.title].filter(Boolean).join(' · ')) +
    '</div><div class="laiso-row" style="margin-top:10px;">' +
    '<input class="laiso-input laiso-grow" inputmode="decimal" placeholder="BPM" data-action="fit-manual-bpm" data-fit-key="' +
    key +
    '">' +
    '<input class="laiso-input laiso-grow" placeholder="Camelot 9A" data-action="fit-manual-camelot" data-fit-key="' +
    key +
    '"></div></div>'
  );
}
function viewFitCheck() {
  var u = state.ui || {};
  var result = u.fitCheckResult;
  var scores = result && result.scores;
  var breakdown = (result && result.breakdown) || {};
  var release = (result && result.release) || {};
  var market = release.marketplace || {};
  var confidenceLabel = {
    high: 'данных хватает',
    medium: 'часть данных',
    low: 'мало данных',
  }[(scores && scores.confidence) || 'low'];
  var candidates = (u.fitCheckCandidates || []).length
    ? '<div class="laiso-mod-label">Нашёл несколько релизов. Выбери нужный.</div><div class="laiso-stack-sm">' +
      u.fitCheckCandidates.map(renderFitCheckCandidate).join('') +
      '</div>'
    : '';
  var enrichedCount = breakdown.enriched_count || 0;
  var releaseTrackCount = result ? result.release_track_count || 0 : 0;
  var matchedCount = breakdown.matched_track_count || 0;
  var coveragePercent = Math.round((breakdown.metadata_coverage || 0) * 100);
  var discogsStat = release.rating ? release.rating + '/5' : 'нет рейтинга';
  var discogsSub = release.rating ? (release.rating_count || 0) + ' голосов' : 'Discogs';
  var discogsPrice = market.average_price != null ? market.average_price : market.lowest_price;
  var marketStat =
    discogsPrice != null ? discogsPrice + ' ' + (market.currency || '') : 'нет данных';
  var marketSub =
    discogsPrice != null
      ? (market.average_price != null ? 'средняя цена' : 'минимальная цена') +
        (market.num_for_sale ? ' · ' + market.num_for_sale + ' в продаже' : '')
      : 'нет цены';
  var coverUrl = release.cover_url || release.coverUrl || '';
  var aiBlock =
    u.fitCheckAiVerdict || u.fitCheckAiError || u.fitCheckAiLoading
      ? '<div class="vertax-fit-ai-box"><strong>AI DJ-разбор</strong>' +
        (u.fitCheckAiLoading
          ? '<p><span class="laiso-spinner"></span> Думаю по делу…</p>'
          : u.fitCheckAiError
            ? '<p class="vertax-fit-ai-error">' + esc(u.fitCheckAiError) + '</p>'
            : renderFitCheckAiText(u.fitCheckAiVerdict)) +
        '</div>'
      : '';
  var resultHtml = result
    ? '<div class="laiso-mod-label">результат</div><div class="laiso-panel">' +
      '<div class="vertax-fit-release-head">' +
      '<div class="vertax-fit-release-cover">' +
      (coverUrl ? '<img src="' + esc(coverUrl) + '" alt="">' : '<span>vinyl</span>') +
      '</div><div class="vertax-fit-release-main">' +
      '<div class="laiso-h2">' +
      esc(release.title || '—') +
      '</div><div class="laiso-meta" style="margin-top:4px;">' +
      esc(
        [release.artist, release.label, release.year, release.catalog_number]
          .filter(Boolean)
          .join(' · ')
      ) +
      '</div></div></div><div class="laiso-lcd" style="margin-top:14px;"><div class="laiso-lcd-label">compatibility</div><div class="laiso-lcd-xl">' +
      esc(scores.compatibility_score) +
      '</div><div class="laiso-lcd-label">' +
      esc(scores.scale_label) +
      '</div></div><div class="vertax-fit-summary-grid">' +
      '<div class="vertax-fit-stat"><span>Данные</span><strong>' +
      esc(confidenceLabel) +
      '</strong><small>BPM/Key: ' +
      esc(enrichedCount) +
      ' из ' +
      esc(releaseTrackCount) +
      '</small></div><div class="vertax-fit-stat"><span>Пары</span><strong>' +
      esc(matchedCount) +
      '</strong><small>покрытие ' +
      esc(coveragePercent) +
      '%</small></div><div class="vertax-fit-stat"><span>К покупке</span><strong>' +
      esc(scores.purchase_score) +
      '/100</strong><small>музыка + рейтинг</small></div><div class="vertax-fit-stat"><span>Discogs</span><strong>' +
      esc(discogsStat) +
      '</strong><small>' +
      esc(discogsSub) +
      '</small></div><div class="vertax-fit-stat"><span>Цена на Discogs</span><strong>' +
      esc(marketStat) +
      '</strong><small>' +
      esc(marketSub) +
      '</small></div></div>' +
      renderFitCheckAutoSummary(result.auto_summary) +
      '</div><div class="laiso-mod-label">лучшие совпадения</div><div class="laiso-stack-sm">' +
      ((result.matches || []).slice(0, 8).map(renderFitCheckMatch).join('') ||
        '<div class="laiso-empty">Сильных совпадений не найдено.</div>') +
      '</div>' +
      ((result.unmatched_release_tracks || []).length
        ? '<details class="laiso-panel"><summary class="laiso-h2">Не вписались</summary><div class="laiso-stack-sm" style="margin-top:10px;">' +
          result.unmatched_release_tracks
            .slice(0, 12)
            .map(function (t) {
              return (
                '<div class="laiso-hint">' +
                esc(
                  [t.position, t.title, t.bpm && t.bpm + ' BPM', t.camelot]
                    .filter(Boolean)
                    .join(' · ')
                ) +
                '</div>'
              );
            })
            .join('') +
          '</div></details>'
        : '') +
      ((result.tracks_not_enriched || []).length
        ? '<details class="laiso-panel" open><summary class="laiso-h2">Не нашли BPM/Key</summary><p class="laiso-hint" style="margin:10px 0;">Не удалось найти метаданные для ' +
          result.tracks_not_enriched.length +
          ' треков. Можно заполнить BPM/Key вручную и пересчитать анализ.</p>' +
          result.tracks_not_enriched.map(renderFitCheckManualTrack).join('') +
          '<button class="laiso-btn laiso-btn-block" data-action="fit-check-recalculate">Пересчитать анализ</button></details>'
        : '') +
      '<button class="laiso-btn laiso-btn-secondary laiso-btn-block vertax-fit-ai-btn" data-action="fit-check-ai-verdict" ' +
      (u.fitCheckAiLoading ? 'disabled' : '') +
      '>' +
      (u.fitCheckAiLoading ? '<span class="laiso-spinner"></span> Думаю…' : 'AI DJ-разбор') +
      '</button>' +
      aiBlock
    : '';
  return (
    renderHeader('Подойдёт ли пластинка?') +
    '<div class="laiso-panel"><div class="laiso-h2">Подойдёт ли эта пластинка к моей коллекции?</div>' +
    '<p style="color:var(--text-secondary);line-height:1.55;">Введи название релиза или каталожный номер. Vertax найдёт пластинку в Discogs, подтянет треки, BPM и Key, сравнит с твоей коллекцией и покажет, насколько релиз ложится в твой музыкальный контекст.</p>' +
    '<p class="laiso-hint">Скор считается математически: BPM, Camelot, плотность совпадений и покрытие метаданных. AI DJ-разбор можно включить отдельно.</p>' +
    '<div class="laiso-search" style="margin-top:12px;"><input class="laiso-input" type="search" placeholder="Название релиза или каталожный номер" value="' +
    esc(u.fitCheckQuery || '') +
    '" data-action="fit-check-input"></div>' +
    '<button class="laiso-btn laiso-btn-block" data-action="fit-check-submit" style="margin-top:10px;">' +
    (u.fitCheckLoading ? '<span class="laiso-spinner"></span> Проверяю…' : 'Проверить пластинку') +
    '</button>' +
    (u.fitCheckError
      ? '<div class="laiso-hint laiso-hint-warn" style="margin-top:8px;">' +
        esc(u.fitCheckError) +
        '</div>'
      : '') +
    '</div>' +
    candidates +
    resultHtml +
    renderFooter()
  );
}
/* ---------- DIG / COLLECTION GAPS ---------- */
var DIG_CAMELOT_CELLS = (function () {
  var out = [];
  ['A', 'B'].forEach(function (mode) {
    for (var i = 1; i <= 12; i++) out.push(i + mode);
  });
  return out;
})();
var DIG_BPM_BANDS = [
  { range: '70-89', min: 70, max: 89, genre: 'downtempo / halftime' },
  { range: '90-109', min: 90, max: 109, genre: 'trip-hop / slow breaks' },
  { range: '110-119', min: 110, max: 119, genre: 'disco / slow house' },
  { range: '120-129', min: 120, max: 129, genre: 'house / techno' },
  { range: '130-139', min: 130, max: 139, genre: 'electro / breaks' },
  { range: '140-149', min: 140, max: 149, genre: 'dubstep / grime / UKG' },
  { range: '150-159', min: 150, max: 159, genre: 'footwork / juke' },
  { range: '160-169', min: 160, max: 169, genre: 'jungle / fast breaks' },
  { range: '170-179', min: 170, max: 179, genre: 'drum & bass / jungle' },
  { range: '180-189', min: 180, max: 189, genre: 'fast dnb / breakcore' },
  { range: '190+', min: 190, max: 260, genre: 'hardcore / footwork' },
];
function digNormalizeCamelot(value) {
  var text = String(value || '')
    .trim()
    .toUpperCase();
  return /^(1[0-2]|[1-9])[AB]$/.test(text) ? text : null;
}
function digCleanBpm(value) {
  var n = Number(value);
  return Number.isFinite(n) && n > 0 ? Math.round(n * 10) / 10 : null;
}
function digTrackBpm(track) {
  return digCleanBpm(
    track && (track.manualBpm || track.bpmManual || track.bpm_override || track.bpm)
  );
}
function digTrackCamelot(track) {
  return digNormalizeCamelot(
    track && (track.manualCamelot || track.camelotManual || track.camelot_override || track.camelot)
  );
}
function digFlattenCollection(collection) {
  var tracks = [];
  (Array.isArray(collection) ? collection : []).forEach(function (record) {
    var list = Array.isArray(record && record.tracklist) ? record.tracklist : null;
    if (!list && record && record.title) list = [record];
    (list || []).forEach(function (track) {
      if (!track || !track.title) return;
      tracks.push({
        title: track.title || '',
        artist: track.artist || record.artist || '',
        bpm: digTrackBpm(track),
        camelot: digTrackCamelot(track),
        genre: track.genre || track.style || record.genre || record.style || null,
        label: track.label || record.label || '',
        record_title: record.title || track.record_title || '',
      });
    });
  });
  return tracks;
}
function digConfidence(recordCount, trackCount, metadataCoverage) {
  if (recordCount < 5) return 'none';
  if (recordCount < 20 || trackCount < 30 || metadataCoverage < 0.5) return 'low';
  if (recordCount < 50 || trackCount < 100 || metadataCoverage < 0.75) return 'medium';
  return 'high';
}
function digConfidenceLabel(confidence) {
  return (
    {
      none: 'данных недостаточно',
      low: 'оценка очень приблизительная',
      medium: 'оценка рабочая, но не абсолютная',
      high: 'оценка надёжная',
    }[confidence] || 'данных недостаточно'
  );
}
function digCamelotLevel(count) {
  if (count <= 0) return 'critical';
  if (count <= 2) return 'weak';
  if (count <= 5) return 'ok';
  return 'strong';
}
function digNeighborCells(cell) {
  var cam = digNormalizeCamelot(cell);
  if (!cam) return [];
  var n = parseInt(cam, 10);
  var mode = cam.slice(-1);
  var other = mode === 'A' ? 'B' : 'A';
  var prev = n === 1 ? 12 : n - 1;
  var next = n === 12 ? 1 : n + 1;
  return [n + other, prev + mode, next + mode];
}
function digGapPriority(level, count, neighborsDensity) {
  if (level === 'critical' && count === 0) return neighborsDensity >= 5 ? 'medium' : 'high';
  if (level === 'weak') return neighborsDensity >= 5 ? 'low' : 'medium';
  return null;
}
function digPriorityRank(value) {
  var rank = { high: 0, medium: 1, low: 2 };
  return rank[value] == null ? 9 : rank[value];
}
function digLevelRank(value) {
  var rank = { critical: 0, weak: 1, ok: 2, strong: 3 };
  return rank[value] == null ? 9 : rank[value];
}
function digBriefPrefix(confidence) {
  if (confidence === 'none') return 'Данных пока мало, но ';
  if (confidence === 'low') return 'Предварительно видно: ';
  return '';
}
function computeDigAnalysis(collection) {
  var records = Array.isArray(collection) ? collection : [];
  var tracks = digFlattenCollection(records);
  var enriched = tracks.filter(function (track) {
    return track.bpm && track.camelot;
  });
  var trackCount = tracks.length;
  var coverage = trackCount ? enriched.length / trackCount : 0;
  var confidence = digConfidence(records.length, trackCount, coverage);
  var camelotCounts = {};
  DIG_CAMELOT_CELLS.forEach(function (cell) {
    camelotCounts[cell] = 0;
  });
  enriched.forEach(function (track) {
    if (track.camelot) camelotCounts[track.camelot] = (camelotCounts[track.camelot] || 0) + 1;
  });
  var camelotGrid = {};
  DIG_CAMELOT_CELLS.forEach(function (cell) {
    var count = camelotCounts[cell] || 0;
    var level = digCamelotLevel(count);
    var neighbors = digNeighborCells(cell);
    var neighborsDensity = neighbors.reduce(function (sum, item) {
      return sum + (camelotCounts[item] || 0);
    }, 0);
    var strongestNeighbor = neighbors
      .map(function (item) {
        return { camelot: item, count: camelotCounts[item] || 0 };
      })
      .sort(function (a, b) {
        return b.count - a.count;
      })[0] || { camelot: null, count: 0 };
    var priority = digGapPriority(level, count, neighborsDensity);
    camelotGrid[cell] = {
      count: count,
      level: level,
      priority: priority,
      neighbors_density: neighborsDensity,
      strongest_neighbor: strongestNeighbor,
    };
  });
  var soft = digBriefPrefix(confidence);
  var camelotGaps = DIG_CAMELOT_CELLS.map(function (cell) {
    var data = camelotGrid[cell];
    if (!data.priority) return null;
    var brief =
      data.level === 'critical'
        ? soft +
          'У тебя ' +
          data.strongest_neighbor.camelot +
          '=' +
          data.strongest_neighbor.count +
          ' треков, но ' +
          cell +
          '=0. Сложно делать гармонические переходы — добери ' +
          cell +
          '.'
        : soft + 'В ' + cell + ' только ' + data.count + ' трек(а) — для манёвра мало.';
    return {
      camelot: cell,
      count: data.count,
      level: data.level,
      priority: data.priority,
      neighbors_density: data.neighbors_density,
      strongest_neighbor: data.strongest_neighbor,
      brief: brief,
    };
  })
    .filter(Boolean)
    .sort(function (a, b) {
      return (
        digPriorityRank(a.priority) - digPriorityRank(b.priority) ||
        digLevelRank(a.level) - digLevelRank(b.level) ||
        a.count - b.count ||
        a.neighbors_density - b.neighbors_density
      );
    });
  var bpmValues = tracks
    .map(function (track) {
      return track.bpm;
    })
    .filter(function (bpm) {
      return Number.isFinite(Number(bpm)) && Number(bpm) > 0;
    })
    .map(Number);
  var bpmHistogram = DIG_BPM_BANDS.map(function (band) {
    return {
      range: band.range,
      min: band.min,
      max: band.max,
      genre: band.genre,
      count: bpmValues.filter(function (value) {
        return value >= band.min && value <= band.max;
      }).length,
      isolated_gap: false,
    };
  });
  var bpmGaps = [];
  for (var i = 1; i < bpmHistogram.length - 1; i++) {
    var row = bpmHistogram[i];
    var left = bpmHistogram[i - 1];
    var right = bpmHistogram[i + 1];
    if (
      row.count === 0 &&
      ((left.count >= 5 && right.count >= 3) || (left.count >= 3 && right.count >= 5))
    ) {
      row.isolated_gap = true;
      bpmGaps.push({
        range: row.range,
        genre: row.genre,
        left_density: left.count,
        right_density: right.count,
        brief:
          soft +
          'В диапазоне ' +
          row.range +
          ' BPM (' +
          row.genre +
          ') провал между двумя плотными зонами (' +
          left.count +
          ' слева, ' +
          right.count +
          ' справа).',
      });
    }
  }
  var briefs = [];
  if (coverage < 0.5 && trackCount) {
    briefs.push(
      soft +
        'У ' +
        Math.round((1 - coverage) * 100) +
        '% треков нет Camelot или BPM — анализ неполный. Сначала стоит обогатить коллекцию метаданными.'
    );
  } else if (coverage < 0.75 && trackCount) {
    briefs.push(
      soft +
        'У ' +
        Math.round((1 - coverage) * 100) +
        '% треков нет Camelot или BPM — выводы пока приблизительные.'
    );
  }
  camelotGaps
    .filter(function (gap) {
      return gap.level === 'critical';
    })
    .forEach(function (gap) {
      briefs.push(gap.brief);
    });
  bpmGaps.forEach(function (gap) {
    briefs.push(gap.brief);
  });
  camelotGaps
    .filter(function (gap) {
      return gap.level === 'weak';
    })
    .forEach(function (gap) {
      briefs.push(gap.brief);
    });
  return {
    record_count: records.length,
    track_count: trackCount,
    enriched_track_count: enriched.length,
    metadata_coverage: Math.round(coverage * 100) / 100,
    confidence: confidence,
    camelot_grid: camelotGrid,
    camelot_gaps: camelotGaps,
    bpm_histogram: bpmHistogram,
    bpm_gaps: bpmGaps,
    briefs: briefs,
  };
}
if (typeof window !== 'undefined') window.computeDigAnalysis = computeDigAnalysis;
function renderDigOnboarding(mode, recordCount) {
  var empty = mode === 'empty';
  return (
    '<div class="dig-onboarding" data-testid="' +
    (empty ? 'dig-onboarding-empty' : 'dig-onboarding-too-small') +
    '"><div class="dig-card"><h2>' +
    (empty ? 'Коллекция пока пустая' : 'Данных пока мало') +
    '</h2><p>' +
    (empty
      ? 'Чтобы анализировать коллекцию, Vertax должен её знать. Добавь несколько пластинок вручную или импортируй коллекцию из Discogs.'
      : 'В коллекции ' +
        esc(recordCount) +
        ' пластинок. Этого мало для полезных рекомендаций. Можно добавить ещё пластинки или импортировать коллекцию из Discogs. Если хочешь просто посмотреть механику — покажем анализ, но уверенность будет очень низкой.') +
    '</p><div class="dig-actions"><button class="laiso-btn laiso-btn-main" data-action="go-add">Добавить пластинку</button><button class="laiso-btn laiso-btn-secondary" data-action="goto-discogs-import">Импортировать из Discogs</button>' +
    (empty
      ? ''
      : '<button class="laiso-btn laiso-btn-secondary" data-action="dig-force-show" data-testid="dig-force-show">Показать всё равно</button>') +
    '</div></div></div>'
  );
}
function renderDigBriefs(analysis) {
  var ui = (state.ui && state.ui.dig) || {};
  var briefs = ui.show_all_briefs ? analysis.briefs : analysis.briefs.slice(0, 5);
  return (
    '<section class="dig-briefs"><h2>Что важно</h2>' +
    (briefs.length
      ? '<ul>' +
        briefs
          .map(function (brief) {
            return '<li>' + esc(brief) + '</li>';
          })
          .join('') +
        '</ul>'
      : '<p class="dig-muted">Явных провалов не видно. Похоже, ящик уже довольно ровный.</p>') +
    (!ui.show_all_briefs && analysis.briefs.length > 5
      ? '<button class="dig-link-btn" data-action="dig-show-all-briefs">Показать все</button>'
      : '') +
    '</section>'
  );
}
function renderDigCamelotGrid(analysis) {
  var head = '<span class="dig-camelot-axis"></span>';
  for (var i = 1; i <= 12; i++) head += '<span class="dig-camelot-axis">' + i + '</span>';
  function row(mode) {
    var html = '<span class="dig-camelot-axis dig-camelot-mode">' + mode + '</span>';
    for (var n = 1; n <= 12; n++) {
      var cell = n + mode;
      var data = analysis.camelot_grid[cell] || { count: 0, level: 'critical' };
      html +=
        '<button class="dig-camelot-cell dig-camelot-cell--' +
        esc(data.level) +
        '" data-action="dig-camelot-cell" data-camelot="' +
        esc(cell) +
        '"><span class="dig-camelot-key">' +
        esc(cell) +
        '</span><span class="dig-camelot-count">' +
        esc(data.count) +
        '</span></button>';
    }
    return html;
  }
  return (
    '<div class="dig-camelot-grid-wrap"><div class="dig-camelot-grid" data-testid="dig-camelot-grid">' +
    head +
    row('A') +
    row('B') +
    '</div></div>'
  );
}
function digRenderCandidateStatusMap() {
  try {
    return JSON.parse(localStorage.getItem('vertax_candidate_status') || '{}') || {};
  } catch (_) {
    return {};
  }
}
function digRenderGapKey(gap) {
  if (!gap) return '';
  if (gap.type === 'bpm' || gap.bpm_range)
    return 'bpm:' + (gap.bpm_range || gap.target_bpm_range || '');
  return 'camelot:' + (gap.camelot || '');
}
function digRenderGroupsByGap(result) {
  var map = {};
  ((result && result.groups) || []).forEach(function (group) {
    map[digRenderGapKey(group.gap)] = group;
  });
  return map;
}
function renderDigCandidateCard(candidate, bucket) {
  var statuses = digRenderCandidateStatusMap();
  var status =
    statuses[String(candidate.discogs_id)] && statuses[String(candidate.discogs_id)].status;
  if (status === 'hidden' || status === 'owned') return '';
  var score = candidate.scores && candidate.scores.compatibility_score;
  var market = candidate.marketplace || {};
  var price = market.average_price || market.lowest_price;
  var marketText = price
    ? (market.average_price ? 'средняя ' : 'от ') +
      price +
      (market.currency ? ' ' + market.currency : '')
    : 'цена неизвестна';
  var title = [candidate.artist, candidate.title].filter(Boolean).join(' — ');
  return (
    '<article class="candidate-card candidate-card--' +
    esc(bucket) +
    (status === 'wishlist' ? ' candidate-card--saved' : '') +
    '" data-discogs-id="' +
    esc(candidate.discogs_id) +
    '">' +
    (candidate.cover_url
      ? '<img class="candidate-cover" src="' + esc(candidate.cover_url) + '" alt="">'
      : '<div class="candidate-cover candidate-cover--empty"></div>') +
    '<div class="candidate-main"><div class="candidate-kicker">' +
    esc(bucket === 'strong' ? 'Strong fit' : bucket === 'probable' ? 'Probable fit' : 'Explore') +
    (status === 'wishlist' ? ' · wishlist' : '') +
    '</div><h3>' +
    esc(title || 'Discogs release #' + candidate.discogs_id) +
    '</h3><p class="candidate-meta">' +
    esc([candidate.label, candidate.year, candidate.catalog_number].filter(Boolean).join(' · ')) +
    '</p><div class="candidate-score"><strong>' +
    esc(score == null ? '—' : score) +
    '</strong><span>совместимость</span></div><ul class="candidate-why">' +
    ((candidate.why || [])
      .map(function (item) {
        return '<li>' + esc(item) + '</li>';
      })
      .join('') || '<li>Есть пересечение с текущим gap.</li>') +
    (candidate.scores && candidate.scores.purchase_score
      ? '<li>Реко к покупке ' + esc(candidate.scores.purchase_score) + '/100</li>'
      : '') +
    (market.lowest_price || market.num_for_sale
      ? '<li>Цена на Discogs: ' +
        esc(marketText) +
        (market.num_for_sale ? ', ' + esc(market.num_for_sale) + ' в продаже' : '') +
        '</li>'
      : '') +
    '</ul><div class="candidate-actions"><button class="laiso-btn laiso-btn-sm laiso-btn-main" data-action="candidate-analyze">Анализировать</button><button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="candidate-wishlist">В wishlist</button><button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="candidate-hide">Скрыть</button><button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="candidate-owned">Уже есть</button></div></div></article>'
  );
}
function renderDigCandidatesForGap(group) {
  if (!group) return '';
  var strong = (group.strong || []).map(function (candidate) {
    return renderDigCandidateCard(candidate, 'strong');
  });
  var probable = (group.probable || []).map(function (candidate) {
    return renderDigCandidateCard(candidate, 'probable');
  });
  var explore = (group.explore || []).map(function (candidate) {
    return renderDigCandidateCard(candidate, 'explore');
  });
  var cards = strong.concat(probable, explore).filter(Boolean);
  if (!cards.length) return '';
  return (
    '<div class="dig-gap-candidates"><div class="dig-gap-candidates-head"><strong>Кандидаты</strong><span>' +
    esc(group.match_scope || '') +
    '</span></div><p class="dig-muted">' +
    esc(group.summary || '') +
    '</p><div class="candidate-list">' +
    cards.join('') +
    '</div></div>'
  );
}
function renderDigCandidatesControls(analysis) {
  var ui = (state.ui && state.ui.dig) || {};
  if ((analysis.record_count || 0) < 5) return '';
  var totalGroups =
    ui.candidates_result && ui.candidates_result.groups ? ui.candidates_result.groups.length : 0;
  var totalEmpty =
    ui.candidates_result && ui.candidates_result.empty_groups
      ? ui.candidates_result.empty_groups.length
      : 0;
  return (
    '<section class="dig-candidates-panel"><div><h2>Кандидаты-релизы</h2><p>' +
    ((analysis.record_count || 0) < 20
      ? 'Коллекция ещё небольшая, поэтому рекомендации будут осторожными.'
      : 'Можно подобрать конкретные пластинки из серверной базы кандидатов под самые заметные gaps.') +
    '</p></div><button class="laiso-btn laiso-btn-main" data-action="dig-load-candidates" data-testid="dig-load-candidates" ' +
    (ui.candidates_loading ? 'disabled' : '') +
    '>' +
    (ui.candidates_loading ? '<span class="laiso-spinner"></span> Ищу…' : 'Найти кандидатов') +
    '</button>' +
    (ui.candidates_error
      ? '<p class="laiso-hint laiso-hint-warn">' + esc(ui.candidates_error) + '</p>'
      : '') +
    (ui.candidates_result && !totalGroups
      ? '<p class="dig-muted">' +
        esc(
          totalEmpty
            ? 'Пока не нашли конкретных релизов под эти gaps. База кандидатов ещё пополняется.'
            : 'База кандидатов ещё не заполнена.'
        ) +
        '</p>'
      : '') +
    (ui.candidates_result && totalGroups
      ? '<p class="dig-muted">Найдено групп с кандидатами: ' + esc(totalGroups) + '.</p>'
      : '') +
    '</section>'
  );
}
function renderDigGapCard(gap, group) {
  return (
    '<article class="dig-gap-card" data-testid="dig-gap-card" data-camelot="' +
    esc(gap.camelot) +
    '"><header class="dig-gap-header"><span class="dig-gap-cell">' +
    esc(gap.camelot) +
    '</span><span class="dig-gap-level dig-gap-level--' +
    esc(gap.level) +
    '">' +
    esc(gap.level) +
    '</span>' +
    (gap.priority
      ? '<span class="dig-gap-priority dig-gap-priority--' +
        esc(gap.priority) +
        '">' +
        esc(gap.priority) +
        '</span>'
      : '') +
    '</header><p class="dig-gap-brief">' +
    esc(gap.brief) +
    '</p>' +
    renderDigCandidatesForGap(group) +
    '</article>'
  );
}
function renderDigBpm(analysis) {
  var maxCount = analysis.bpm_histogram.reduce(function (max, row) {
    return Math.max(max, row.count);
  }, 0);
  return (
    '<div class="dig-bpm-histogram" data-testid="dig-bpm-histogram">' +
    (analysis.bpm_histogram.length
      ? analysis.bpm_histogram
          .map(function (row) {
            var scale = maxCount ? Math.max(row.count / maxCount, row.count ? 0.08 : 0) : 0;
            return (
              '<div class="dig-bpm-row' +
              (row.isolated_gap ? ' dig-bpm-row--gap' : '') +
              '" data-bpm-range="' +
              esc(row.range) +
              '"><div class="dig-bpm-label"><span class="dig-bpm-range">' +
              esc(row.range) +
              ' BPM</span><span class="dig-bpm-genre">' +
              esc(row.genre || '') +
              '</span></div><div class="dig-bpm-bar-wrap"><div class="dig-bpm-bar" style="--bar-scale:' +
              esc(scale) +
              '"></div></div><span class="dig-bpm-count">' +
              esc(row.count) +
              '</span></div>'
            );
          })
          .join('')
      : '<p class="dig-muted">Пока нет BPM-данных для гистограммы.</p>') +
    '</div>'
  );
}
function renderDigAnalysis(analysis) {
  var coveragePct = Math.round((analysis.metadata_coverage || 0) * 100);
  var groupsByGap = digRenderGroupsByGap(
    state.ui && state.ui.dig && state.ui.dig.candidates_result
  );
  return (
    '<section class="dig-meta"><div class="dig-confidence dig-confidence--' +
    esc(analysis.confidence) +
    '"><span>Анализ: ' +
    esc(analysis.record_count) +
    ' пластинок, ' +
    esc(analysis.track_count) +
    ' треков. BPM/Key найден у ' +
    esc(analysis.enriched_track_count) +
    '.</span><span class="dig-confidence-label">Уверенность: ' +
    esc(digConfidenceLabel(analysis.confidence)) +
    ' · покрытие ' +
    esc(coveragePct) +
    '%</span></div></section>' +
    renderDigBriefs(analysis) +
    renderDigCandidatesControls(analysis) +
    '<section class="dig-camelot"><h2>Camelot</h2>' +
    renderDigCamelotGrid(analysis) +
    '<div class="dig-camelot-gaps">' +
    (analysis.camelot_gaps.length
      ? analysis.camelot_gaps
          .slice(0, 12)
          .map(function (gap) {
            return renderDigGapCard(
              gap,
              groupsByGap[digRenderGapKey({ type: 'camelot', camelot: gap.camelot })]
            );
          })
          .join('')
      : '<p class="dig-muted">Критичных Camelot-провалов не видно.</p>') +
    '</div></section><section class="dig-bpm"><h2>BPM</h2>' +
    renderDigBpm(analysis) +
    '<div class="dig-bpm-gaps">' +
    (analysis.bpm_gaps.length
      ? analysis.bpm_gaps
          .map(function (gap) {
            return (
              '<article class="dig-gap-card" data-testid="dig-gap-card" data-bpm-range="' +
              esc(gap.range) +
              '"><header class="dig-gap-header"><span class="dig-gap-cell">' +
              esc(gap.range) +
              ' BPM</span></header><p class="dig-gap-brief">' +
              esc(gap.brief) +
              '</p>' +
              renderDigCandidatesForGap(
                groupsByGap[digRenderGapKey({ type: 'bpm', bpm_range: gap.range })]
              ) +
              '</article>'
            );
          })
          .join('')
      : '<p class="dig-muted">Изолированных BPM-провалов не видно.</p>') +
    '</div></section>'
  );
}
function viewDig() {
  var collection = state.collection || [];
  var ui = (state.ui = state.ui || {});
  ui.dig = ui.dig || { force_show: false, show_all_briefs: false };
  var recordCount = collection.length;
  var forceShow = !!ui.dig.force_show;
  var body = '';
  if (recordCount === 0) body = renderDigOnboarding('empty', recordCount);
  else if (recordCount < 5 && !forceShow) body = renderDigOnboarding('too_small', recordCount);
  else body = renderDigAnalysis(computeDigAnalysis(collection));
  return (
    renderChassis() +
    '<div class="dig-view" data-testid="dig-view"><header class="dig-header"><button class="laiso-back" data-action="back">← Назад</button><h1>Что докопать</h1></header>' +
    body +
    '</div>' +
    renderFooter()
  );
}
/* ---------- COLLECTION & SETTINGS ---------- */ function viewCollection() {
  var q = (state.ui.collectionFilter || '').toLowerCase();
  var sortMode = state.ui.collectionSort || 'recent';
  var allVinyls = state.collection.slice();
  function vinylBpmStat(v) {
    var ts = (v.tracklist || [])
      .filter(function (t) {
        return t.bpm;
      })
      .map(function (t) {
        return t.bpm;
      });
    if (!ts.length) return null;
    return (
      ts.reduce(function (a, b) {
        return a + b;
      }, 0) / ts.length
    );
  }
  function vinylCamStat(v) {
    var ts = (v.tracklist || [])
      .filter(function (t) {
        return t.camelot;
      })
      .map(function (t) {
        return t.camelot;
      });
    return ts[0] || null;
  }
  if (sortMode === 'title') {
    allVinyls.sort(function (a, b) {
      return String(a.title || '').localeCompare(String(b.title || ''));
    });
  } else if (sortMode === 'artist') {
    allVinyls.sort(function (a, b) {
      return String(a.artist || '').localeCompare(String(b.artist || ''));
    });
  } else if (sortMode === 'bpm-asc' || sortMode === 'bpm-desc') {
    allVinyls.sort(function (a, b) {
      var aa = vinylBpmStat(a),
        bb = vinylBpmStat(b);
      if (aa === null && bb === null) return 0;
      if (aa === null) return 1;
      if (bb === null) return -1;
      return sortMode === 'bpm-asc' ? aa - bb : bb - aa;
    });
  } else if (sortMode === 'camelot') {
    allVinyls.sort(function (a, b) {
      var aa = vinylCamStat(a),
        bb = vinylCamStat(b);
      if (!aa && !bb) return 0;
      if (!aa) return 1;
      if (!bb) return -1;
      var na = parseInt(aa, 10),
        nb = parseInt(bb, 10);
      if (na !== nb) return na - nb;
      return aa.slice(-1).localeCompare(bb.slice(-1));
    });
  } else if (sortMode === 'vinyl') {
    allVinyls.sort(function (a, b) {
      return String(a.catno || '').localeCompare(String(b.catno || ''));
    });
  } else {
    /* recent */ allVinyls.sort(function (a, b) {
      return (b.addedAt || 0) - (a.addedAt || 0);
    });
  }
  var filtered = q
    ? allVinyls.filter(function (v) {
        return [v.title, v.artist, v.label, v.catno, v.year, v.format].some(function (f) {
          return (
            String(f || '')
              .toLowerCase()
              .indexOf(q) >= 0
          );
        });
      })
    : allVinyls;
  var sortLabels = {
    recent: 'Недавние',
    title: 'A–Z по названию',
    artist: 'A–Z по артисту',
    'bpm-asc': 'BPM ↑',
    'bpm-desc': 'BPM ↓',
    camelot: 'Camelot',
    vinyl: 'Каталог',
  };
  var sortHtml =
    '<div class="laiso-row" style="gap:6px;flex-wrap:wrap;margin:6px 0;">' +
    Object.keys(sortLabels)
      .map(function (k) {
        return (
          '<button class="laiso-chip ' +
          (sortMode === k ? 'active' : '') +
          '" data-action="collection-sort" data-mode="' +
          k +
          '">' +
          esc(sortLabels[k]) +
          '</button>'
        );
      })
      .join('') +
    '</div>';
  var vinylsHtml = filtered.length
    ? filtered
        .map(function (v) {
          return (
            renderVinylCard(v) +
            (v.excludeFromSets
              ? '<div class="laiso-meta" style="padding:0 4px 6px;color:var(--warning);">[не используется в сетах]</div>'
              : '')
          );
        })
        .join('')
    : '<div class="laiso-empty">Коллекция пуста.<br>Сохрани пластинки на экране «Добавить пластинку».</div>';
  var setsList = state.sets.length
    ? state.sets
        .slice()
        .sort(function (a, b) {
          return (b.createdAt || 0) - (a.createdAt || 0);
        })
        .map(function (s) {
          return (
            '<div class="laiso-vinyl-card">' +
            '<div class="laiso-vinyl-cover" style="font-size:18px;color:var(--text-secondary);">♪</div>' +
            '<div class="laiso-vinyl-info">' +
            '<div class="laiso-vinyl-title">' +
            esc(s.name) +
            '</div>' +
            '<div class="laiso-vinyl-meta">' +
            esc((s.mode || '').toUpperCase()) +
            ' · ' +
            (s.tracks || []).length +
            ' треков · ' +
            new Date(s.createdAt).toLocaleDateString() +
            '</div>' +
            '</div>' +
            '<div class="laiso-vinyl-actions">' +
            '<button class="laiso-btn laiso-btn-sm laiso-btn-secondary" data-action="set-load" data-id="' +
            esc(s.id) +
            '">Открыть</button>' +
            '<button class="laiso-btn laiso-btn-sm laiso-btn-warning" data-action="set-delete" data-id="' +
            esc(s.id) +
            '">Удал.</button>' +
            '</div>' +
            '</div>'
          );
        })
        .join('')
    : '<div class="laiso-empty">Сохранённых сетов нет</div>';
  return (
    renderHeader('Коллекция') +
    '<div class="laiso-tabs">' +
    '<button class="laiso-tab active">Пластинки (' +
    state.collection.length +
    ')</button>' +
    '<button class="laiso-tab" data-action="collection-jump-sets">Сеты (' +
    state.sets.length +
    ')</button>' +
    '</div>' +
    '<div class="laiso-panel runt18c-safe-build vertax-collection-set-build" style="margin:10px 0 14px;"><div class="laiso-h2" style="margin-bottom:8px;">Собрать сет из коллекции</div><button type="button" class="laiso-btn laiso-btn-block" data-action="runt28-open-source">Выбрать пластинки / вся коллекция</button></div>' +
    '<div class="laiso-search">' +
    '<input class="laiso-input" type="search" placeholder="Поиск: артист / название / лейбл / каталог…" value="' +
    esc(state.ui.collectionFilter || '') +
    '" data-action="collection-search">' +
    '</div>' +
    sortHtml +
    '<div class="laiso-stack-sm">' +
    vinylsHtml +
    '</div>' +
    '<div id="laiso-sets-section" style="margin-top:32px;">' +
    '<div class="laiso-meta" style="margin-bottom:8px;">Сохранённые сеты</div>' +
    '<div class="laiso-stack-sm">' +
    setsList +
    '</div>' +
    '</div>' +
    '<div class="laiso-panel" style="margin-top:32px;">' +
    '<div class="laiso-meta" style="margin-bottom:12px;">Настройки</div>' +
    '<div class="laiso-stack-sm">' +
    '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="export-json">Экспорт коллекции (JSON)</button>' +
    '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="import-json">Импорт коллекции (JSON)</button>' +
    '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="goto-backup">🗂 Резервная копия</button>' +
    '<button class="laiso-btn laiso-btn-warning laiso-btn-block" data-action="confirm-clear">Стереть все данные</button>' +
    '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="open-about">О приложении</button>' +
    '<button class="laiso-btn laiso-btn-secondary laiso-btn-block" data-action="open-help">Справка</button>' +
    '</div>' +
    '</div>' +
    renderFooter()
  );
}
function viewAbout() {
  return (
    renderHeader('О приложении') +
    '<div class="laiso-panel">' +
    '<div class="laiso-h1">VERTAX-01</div>' +
    '<div style="color:var(--text-secondary);margin-top:4px;">by Laiso Buck · цифровой компаньон DJ-винильщика · v0.1</div>' +
    '<p style="margin-top:16px;">Найди релиз в Discogs текстом, подтяни треклист, проставь BPM/Key, собери сет по правилу «не два трека с одной пластинки подряд».</p>' +
    '<p>by <strong>Михаил Проскурин</strong></p>' +
    '</div>' +
    renderFooter()
  );
}

/* RUNT render extensions. Installed from app.js after boot in the original order. */

function installRuntSetManualMetaControls() {
  (function () {
    function patchFindTrackPair(vid, tid) {
      var v = findVinyl(vid);
      var t = v && findTrack(v, tid);
      return { v: v, t: t };
    }
    function syncGeneratedTrack(vid, tid) {
      if (!state.ui || !state.ui.generatedSet) return;
      var p = patchFindTrackPair(vid, tid);
      if (!p.t) return;
      state.ui.generatedSet.forEach(function (row) {
        if (String(row.id) === String(tid) && String(row.recordId) === String(vid)) {
          row.bpm = p.t.bpm || null;
          row.key = p.t.key || null;
          row.camelot = p.t.camelot || null;
          row.originalBpm = p.t.originalBpm || null;
          row.bpmSource = p.t.bpmSource || null;
          row.displayPosition = displayPosition(p.t, p.v);
          row.position = p.t.position;
        }
      });
    }
    function parseManualKey(input) {
      var k = String(input || '').trim();
      if (!k) return { key: null, camelot: null };
      if (/^[0-9]{1,2}[AB]$/i.test(k)) {
        var cam = k.toUpperCase();
        return { key: CAMELOT_TO_KEY[cam] || null, camelot: cam };
      }
      var aliases = {
        am: 'A minor',
        'a min': 'A minor',
        'a minor': 'A minor',
        'a#m': 'A# minor',
        bbm: 'A# minor',
        'a# min': 'A# minor',
        'bb min': 'A# minor',
        bm: 'B minor',
        'b min': 'B minor',
        'b minor': 'B minor',
        cm: 'C minor',
        'c min': 'C minor',
        'c minor': 'C minor',
        'c#m': 'C# minor',
        dbm: 'C# minor',
        'c# min': 'C# minor',
        'db min': 'C# minor',
        dm: 'D minor',
        'd min': 'D minor',
        'd minor': 'D minor',
        'd#m': 'D# minor',
        ebm: 'D# minor',
        'd# min': 'D# minor',
        'eb min': 'D# minor',
        em: 'E minor',
        'e min': 'E minor',
        'e minor': 'E minor',
        fm: 'F minor',
        'f min': 'F minor',
        'f minor': 'F minor',
        'f#m': 'F# minor',
        gbm: 'F# minor',
        'f# min': 'F# minor',
        'gb min': 'F# minor',
        gm: 'G minor',
        'g min': 'G minor',
        'g minor': 'G minor',
        'g#m': 'G# minor',
        abm: 'G# minor',
        'g# min': 'G# minor',
        'ab min': 'G# minor',
        a: 'A major',
        'a maj': 'A major',
        'a major': 'A major',
        'a#': 'A# major',
        bb: 'A# major',
        'a# maj': 'A# major',
        'bb maj': 'A# major',
        b: 'B major',
        'b maj': 'B major',
        'b major': 'B major',
        c: 'C major',
        'c maj': 'C major',
        'c major': 'C major',
        'c#': 'C# major',
        db: 'C# major',
        'c# maj': 'C# major',
        'db maj': 'C# major',
        d: 'D major',
        'd maj': 'D major',
        'd major': 'D major',
        'd#': 'D# major',
        eb: 'D# major',
        'd# maj': 'D# major',
        'eb maj': 'D# major',
        e: 'E major',
        'e maj': 'E major',
        'e major': 'E major',
        f: 'F major',
        'f maj': 'F major',
        'f major': 'F major',
        'f#': 'F# major',
        gb: 'F# major',
        'f# maj': 'F# major',
        'gb maj': 'F# major',
        g: 'G major',
        'g maj': 'G major',
        'g major': 'G major',
        'g#': 'G# major',
        ab: 'G# major',
        'g# maj': 'G# major',
        'ab maj': 'G# major',
      };
      var low = k.toLowerCase().replace(/♯/g, '#').replace(/♭/g, 'b').replace(/\s+/g, ' ').trim();
      var nk = normalizeKey(k) || aliases[low] || null;
      return { key: nk || k, camelot: nk ? KEY_TO_CAMELOT[nk] : null };
    }
    handlers['bpm-x2'] = function (_, el) {
      var vid = el.dataset.vid,
        tid = el.dataset.tid;
      var p = patchFindTrackPair(vid, tid);
      if (!p.t || !p.t.bpm) return;
      p.t.originalBpm = p.t.originalBpm || p.t.bpm;
      p.t.bpm = p.t.bpm * 2;
      p.t.halftimeCorrected = true;
      p.t.bpmSource = 'manual_x2';
      syncGeneratedTrack(vid, tid);
      persistVinyl(p.v);
      showToast('BPM ×2: ' + p.t.bpm);
      render();
    };
    handlers['bpm-divide-2'] = function (_, el) {
      var vid = el.dataset.vid,
        tid = el.dataset.tid;
      var p = patchFindTrackPair(vid, tid);
      if (!p.t || !p.t.bpm) return;
      if (p.t.originalBpm) {
        p.t.bpm = p.t.originalBpm;
        p.t.originalBpm = null;
        p.t.halftimeCorrected = false;
        p.t.bpmSource = 'manual';
      } else {
        p.t.originalBpm = p.t.bpm;
        p.t.bpm = Math.round(p.t.bpm / 2);
        p.t.halftimeCorrected = true;
        p.t.bpmSource = 'manual_div2';
      }
      syncGeneratedTrack(vid, tid);
      persistVinyl(p.v);
      showToast('BPM: ' + p.t.bpm);
      render();
    };
    handlers['track-manual-meta'] = async function (_, el) {
      var vid = el.dataset.vid,
        tid = el.dataset.tid;
      var p = patchFindTrackPair(vid, tid);
      if (!p.t) return;
      var bpmStr = await vertaxPrompt('BPM (60–220) или пусто:', p.t.bpm || '');
      if (bpmStr !== null) {
        if (bpmStr.trim() === '') {
          p.t.bpm = null;
          p.t.bpmSource = null;
          p.t.originalBpm = null;
          p.t.halftimeCorrected = false;
        } else {
          var n = parseInt(bpmStr, 10);
          if (!isNaN(n) && n >= 60 && n <= 220) {
            p.t.bpm = n;
            p.t.bpmSource = 'manual';
            p.t.originalBpm = null;
            p.t.halftimeCorrected = false;
          }
        }
      }
      var keyStr = await vertaxPrompt(
        'Тональность или Camelot (например 6B, 12A, Am, F#m):',
        p.t.camelot || p.t.key || ''
      );
      if (keyStr !== null) {
        var parsed = parseManualKey(keyStr);
        p.t.key = parsed.key;
        p.t.camelot = parsed.camelot;
        p.t.keySource = parsed.key || parsed.camelot ? 'manual' : null;
      }
      p.t.metaStatus = 'manual';
      p.t.confidence = p.t.bpm && (p.t.key || p.t.camelot) ? 'high' : 'manual';
      syncGeneratedTrack(vid, tid);
      persistVinyl(p.v);
      showToast('BPM/Key обновлены');
      render();
    };
    window.renderRuntSetCard = function (tr, idx, arr) {
      var prev = idx > 0 ? arr[idx - 1] : null;
      var dBpm = prev && prev.bpm && tr.bpm ? tr.bpm - prev.bpm : null;
      var sameVinyl =
        prev &&
        ((prev.recordKey && tr.recordKey && prev.recordKey === tr.recordKey) ||
          prev.recordId === tr.recordId);
      var pos = tr.displayPosition || tr.position || '—';
      var dStr = dBpm === null ? '' : ' (' + (dBpm > 0 ? '+' : '') + dBpm + ')';
      var x2 = '';
      if (tr.bpm) {
        if (tr.originalBpm) {
          x2 =
            '<button class="laiso-x2-btn" data-action="bpm-divide-2" data-vid="' +
            esc(tr.recordId) +
            '" data-tid="' +
            esc(tr.id) +
            '">÷2</button>';
        } else {
          x2 =
            '<button class="laiso-x2-btn" data-action="bpm-x2" data-vid="' +
            esc(tr.recordId) +
            '" data-tid="' +
            esc(tr.id) +
            '">×2</button>';
        }
      }
      var warning = sameVinyl
        ? '<div class="laiso-warn-line">⚠ Та же пластинка, что предыдущий трек</div>'
        : '';
      var bpmHint = tr.originalBpm
        ? '<span class="laiso-set-bpm-hint">исх. ' + esc(String(tr.originalBpm)) + ' ×2</span>'
        : '';
      var catnoStr = tr.vinylCatno ? ' · ' + esc(tr.vinylCatno) : '';
      return (
        warning +
        '<div class="laiso-set-card">' +
        '<div class="laiso-set-order">#' +
        (idx + 1) +
        '</div>' +
        '<div class="laiso-set-bigmeta">' +
        '<span class="laiso-set-big-bpm">' +
        (tr.bpm ? esc(String(tr.bpm)) : '—') +
        '</span>' +
        '<span class="laiso-set-big-cam">' +
        esc(tr.camelot || '—') +
        '</span>' +
        (tr.key ? '<span class="laiso-set-big-key">KEY: ' + esc(tr.key) + '</span>' : '') +
        '</div>' +
        '<div class="laiso-set-body">' +
        '<span class="laiso-set-title">' +
        esc(tr.title || '—') +
        '</span>' +
        '<div class="laiso-set-meta">' +
        esc(tr.vinylArtist || '') +
        ' — ' +
        esc(tr.vinylTitle || '') +
        catnoStr +
        '</div>' +
        '<div class="laiso-set-bottom">' +
        '<span class="laiso-set-pos-bottom">' +
        esc(pos) +
        '</span>' +
        '<span class="laiso-pill laiso-pill-bpm">' +
        (tr.bpm ? esc(String(tr.bpm)) + ' BPM' + dStr : '— BPM') +
        '</span>' +
        x2 +
        bpmHint +
        '<button class="laiso-x2-btn" data-action="track-manual-meta" data-vid="' +
        esc(tr.recordId) +
        '" data-tid="' +
        esc(tr.id) +
        '">править</button>' +
        '</div>' +
        '</div>' +
        '</div>'
      );
    };
    var oldViewSet = viewSet;
    viewSet = function () {
      var html = oldViewSet();
      if (!state.ui || !state.ui.generatedSet || !state.ui.generatedSet.length) return html;
      var newRows = state.ui.generatedSet
        .map(function (tr, idx, arr) {
          return window.renderRuntSetCard(tr, idx, arr);
        })
        .join('');
      /* replace only the current generated set stack block; fallback leaves old markup if structure changes */ html =
        html.replace(
          /<div class="laiso-mod-label">сет · [\s\S]*?<div class="laiso-row" style="margin-top:14px;">/,
          function (match) {
            var exportStart = '<div class="laiso-row" style="margin-top:14px;">';
            return (
              '<div class="laiso-mod-label">сет · ' +
              state.ui.generatedSet.length +
              ' трек.</div><div class="laiso-stack-sm">' +
              newRows +
              '</div>' +
              exportStart
            );
          }
        );
      return html;
    };
  })();
}

function installRuntCompactSetCards() {
  (function () {
    if (!window.laisoBuck || !window.laisoBuck.state) return;
    function safeEsc(s) {
      if (typeof esc === 'function') return esc(s);
      if (s == null) return '';
      return String(s).replace(/[&<>"']/g, function (c) {
        return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c];
      });
    }
    function compactSetCard(tr, idx, arr) {
      var prev = idx > 0 ? arr[idx - 1] : null;
      var dBpm = prev && prev.bpm && tr.bpm ? tr.bpm - prev.bpm : null;
      var dStr = dBpm === null ? '' : (dBpm > 0 ? '+' : '') + dBpm;
      var pos = tr.displayPosition || tr.position || '—';
      var bpm = tr.bpm || '—';
      var cam = tr.camelot || '—';
      var release =
        (tr.vinylArtist || '') +
        ' — ' +
        (tr.vinylTitle || '') +
        (tr.vinylCatno ? ' · ' + tr.vinylCatno : '');
      var sameVinyl =
        prev &&
        ((tr.recordKey && prev.recordKey && tr.recordKey === prev.recordKey) ||
          tr.recordId === prev.recordId);
      var x2Button = '';
      if (tr.originalBpm && tr.bpm) {
        x2Button =
          '<button class="laiso-btn laiso-btn-secondary runt-set-btn" data-action="bpm-divide-2" data-vid="' +
          safeEsc(tr.recordId) +
          '" data-tid="' +
          safeEsc(tr.id) +
          '">÷2</button>';
      } else if (tr.bpm && tr.bpm >= 70 && tr.bpm <= 100) {
        x2Button =
          '<button class="laiso-btn laiso-btn-secondary runt-set-btn" data-action="bpm-x2" data-vid="' +
          safeEsc(tr.recordId) +
          '" data-tid="' +
          safeEsc(tr.id) +
          '">×2</button>';
      } else if (tr.bpm && tr.bpm >= 130) {
        x2Button =
          '<button class="laiso-btn laiso-btn-secondary runt-set-btn" data-action="bpm-divide-2" data-vid="' +
          safeEsc(tr.recordId) +
          '" data-tid="' +
          safeEsc(tr.id) +
          '">÷2</button>';
      }
      return (
        (sameVinyl ? '<div class="laiso-warn-line">⚠ Та же пластинка рядом</div>' : '') +
        '<div class="runt-set-card" draggable="true" data-set-idx="' +
        idx +
        '">' +
        '<div class="runt-set-order">#' +
        (idx + 1) +
        '</div>' +
        '<div class="runt-set-left">' +
        '<div class="runt-metric runt-metric-bpm">' +
        safeEsc(bpm) +
        '</div>' +
        '<div class="runt-metric runt-metric-cam">' +
        safeEsc(cam) +
        '</div>' +
        '</div>' +
        '<div class="runt-set-main">' +
        '<h3 class="runt-set-title">' +
        safeEsc(tr.title || '—') +
        '</h3>' +
        '<div class="runt-set-release">' +
        safeEsc(release) +
        '</div>' +
        '<div class="runt-set-actions">' +
        '<span class="runt-chip runt-chip-pos">на пластинке ' +
        safeEsc(pos) +
        '</span>' +
        '<span class="runt-chip runt-chip-bpm">' +
        safeEsc(bpm) +
        ' BPM' +
        (dStr ? '<span class="runt-chip-delta">(' + safeEsc(dStr) + ')</span>' : '') +
        '</span>' +
        x2Button +
        '<button class="laiso-btn laiso-btn-secondary runt-set-btn" data-action="track-manual-meta" data-vid="' +
        safeEsc(tr.recordId) +
        '" data-tid="' +
        safeEsc(tr.id) +
        '">править</button>' +
        '<button class="runt30-remove-track" type="button" data-runt30-remove-index="' +
        idx +
        '">убрать</button>' +
        '</div>' +
        '<div class="runt-drag-hint">↕ перетащить</div>' +
        '</div>' +
        '</div>'
      );
    }
    function renderCompactControls(mode, opts) {
      var modeTabs =
        '<div class="laiso-tabs laiso-tabs-modes">' +
        [
          ['best-flow', 'BEST FLOW'],
          ['tempo-safe', 'ПО ТЕМПУ'],
          ['camelot-safe', 'ПО CAMELOT'],
          ['camelot-filter', 'ФИЛЬТР'],
          ['custom', 'СВОЙ'],
        ]
          .map(function (p) {
            return (
              '<button class="laiso-tab ' +
              (mode === p[0] ? 'active' : '') +
              '" data-action="set-mode" data-mode="' +
              p[0] +
              '">' +
              p[1] +
              '</button>'
            );
          })
          .join('') +
        '</div>';
      var controls = '';
      if (mode === 'tempo-safe' || mode === 'best-flow') {
        controls =
          '<div class="laiso-panel" style="padding:12px!important;">' +
          '<label class="laiso-label">Диапазон темпа ± ' +
          opts.tempoRange +
          ' BPM</label>' +
          '<div class="laiso-toggle">' +
          [2, 4, 6]
            .map(function (n) {
              return (
                '<button class="' +
                (opts.tempoRange === n ? 'active' : '') +
                '" data-action="set-tempo" data-n="' +
                n +
                '">±' +
                n +
                '</button>'
              );
            })
            .join('') +
          '</div>' +
          '</div>';
      } else if (mode === 'camelot-safe') {
        controls =
          '<div class="laiso-panel"><label class="laiso-label">Стартовый Camelot</label>' +
          renderCamelotWheel(opts.targetCamelot) +
          '</div>';
      } else if (mode === 'camelot-filter') {
        var sel = opts.camelotSet || {};
        controls =
          '<div class="laiso-panel"><label class="laiso-label">Выбери Camelot</label>' +
          renderCamelotWheelMulti(sel) +
          '</div>';
      } else if (mode === 'custom') {
        controls =
          '<div class="laiso-panel"><div class="laiso-label">Свой сет</div><p style="margin:0;color:var(--text-secondary);">Добавляй треки из коллекции вручную, меняй порядок перетаскиванием и убирай лишнее прямо из списка.</p></div>';
      }
      return modeTabs + controls;
    }
    window.viewSet = function () {
      var mode = state.ui.setMode;
      var opts = state.ui.setOptions;
      var hasSet = state.ui.generatedSet && state.ui.generatedSet.length > 0;
      var availTracks = getAllSessionTracks();
      var availCount = availTracks.length;
      var missing =
        mode === 'custom'
          ? []
          : getAllSessionTracks({ includeAll: true }).filter(function (t) {
              return !t.trackExcluded && !t.vinylExcluded && (!t.bpm || !t.camelot);
            });
      var missingNotice = '';
      if (missing.length > 0 && !state.ui.setOpenDataPanel) {
        missingNotice =
          '<div class="laiso-panel laiso-data-panel" style="border-color:var(--warning);background:#FFF8F2;">' +
          '<div class="laiso-data-head">' +
          '<strong>Есть треки без BPM / Camelot</strong>' +
          '<span class="laiso-meta">' +
          missing.length +
          ' требуют проверки</span>' +
          '</div>' +
          '<p class="laiso-data-hint">Заполни значения вручную или исключи треки из сета.</p>' +
          '<button class="laiso-btn laiso-btn-sm laiso-btn-block" data-action="toggle-data-panel">Показать и исправить</button>' +
          '</div>';
      }
      var setHtml = '';
      if (hasSet) {
        setHtml =
          '<div class="laiso-mod-label">сет · ' +
          state.ui.generatedSet.length +
          ' трек.</div>' +
          '<div class="runt-set-list">' +
          state.ui.generatedSet.map(compactSetCard).join('') +
          '</div>';
      }
      var primaryRow =
        mode === 'custom'
          ? '<div class="laiso-row" style="margin:14px 0;"><button class="laiso-btn laiso-grow" data-action="open-add-track-modal">Добавить трек из коллекции</button></div>'
          : '<div class="laiso-row" style="margin:14px 0;"><button class="laiso-btn laiso-grow" data-action="set-generate">' +
            (hasSet ? 'Пересобрать' : 'Сгенерировать') +
            '</button></div>';
      var emptyText =
        mode === 'custom' && !hasSet
          ? '<div class="laiso-empty">Свой сет пока пуст. Нажми «Добавить трек из коллекции» и собери порядок руками.</div>'
          : availCount < 2
            ? '<div class="laiso-empty">Нужно минимум 2 трека. Добавь пластинки и проставь BPM/Key.</div>'
            : '';
      return (
        renderHeader('Сборка сета', {
          right: '<span class="laiso-counter">' + availCount + ' трек.</span>',
        }) +
        renderCompactControls(mode, opts) +
        primaryRow +
        emptyText +
        missingNotice +
        setHtml +
        (hasSet
          ? '<div class="laiso-row" style="margin-top:14px;">' +
            '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="add-track-from-collection">+ Добавить трек из коллекции</button>' +
            '<button class="laiso-btn laiso-btn-secondary laiso-grow" data-action="set-export">Экспорт TXT</button>' +
            '<button class="laiso-btn laiso-grow" data-action="set-save">Сохранить сет</button>' +
            '</div>'
          : '')
      );
    };
    /* Drag and drop reorder for compact set cards */ var dragIdx = null;
    document.addEventListener('dragstart', function (e) {
      var card = e.target && e.target.closest && e.target.closest('#laiso-app .runt-set-card');
      if (!card) return;
      dragIdx = parseInt(card.getAttribute('data-set-idx'), 10);
      card.classList.add('runt-dragging');
      if (e.dataTransfer) {
        e.dataTransfer.effectAllowed = 'move';
        e.dataTransfer.setData('text/plain', String(dragIdx));
      }
    });
    document.addEventListener('dragend', function (e) {
      var card = e.target && e.target.closest && e.target.closest('#laiso-app .runt-set-card');
      if (card) card.classList.remove('runt-dragging');
      dragIdx = null;
    });
    document.addEventListener('dragover', function (e) {
      var card = e.target && e.target.closest && e.target.closest('#laiso-app .runt-set-card');
      if (!card) return;
      e.preventDefault();
      if (e.dataTransfer) e.dataTransfer.dropEffect = 'move';
    });
    document.addEventListener('drop', function (e) {
      var card = e.target && e.target.closest && e.target.closest('#laiso-app .runt-set-card');
      if (!card) return;
      e.preventDefault();
      var from = dragIdx;
      if (from === null && e.dataTransfer)
        from = parseInt(e.dataTransfer.getData('text/plain'), 10);
      var to = parseInt(card.getAttribute('data-set-idx'), 10);
      if (isNaN(from) || isNaN(to) || from === to) return;
      var arr = state.ui.generatedSet || [];
      var moved = arr.splice(from, 1)[0];
      arr.splice(to, 0, moved);
      state.ui.generatedSet = arr;
      if (typeof render === 'function') render();
    });
    if (typeof render === 'function') render();
  })();
}
